...
- **Default Cover Art attribute** (attr id `0x08`) in the `GetElementAttributes` response. Currently emits an empty handle (graceful-degradation signal that the CT should skip BIP/OBEX); a follow-up will wire the BIP responder Java layer + populate the per-track 7-character ASCII hex handle so CTs that gate their metadata pane render on a non-empty cover-art handle no longer present an empty pane.
