...
- `mtkbt`: M1 patch flips the AV/C ctype byte emitted for msg=544 RegisterNotification responses from `0x0D` (CHANGED) to `0x0F` (INTERIM) at file `0x37cca`. Stock mtkbt hardcoded CHANGED at this site, which was correct for stock JNI flow (msg=544 was only called for actual value changes); the v2.0 trampoline chain routes both initial INTERIM and proactive CHANGED through msg=544, so without M1 every RegisterNotification response went out as CHANGED-without-INTERIM, violating AVRCP 1.3 §6.7.1 and causing strict CTs to fall back to ~3 s polling indefinitely (metadata pane never rendered). Trade-off: T5 / T9 proactive edge emits also become INTERIM-on-wire — CTs treat repeated INTERIMs as fresh subscription confirmations per §6.7.1, so state refresh continues to work via the INTERIM payload, but the explicit CHANGED edge signal is gone.
