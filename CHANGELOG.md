...
This release lands the **AVRCP 1.3 metadata pipeline**: a peer Bluetooth Controller (car head unit, TV, smart speaker, etc.) now sees Title/Artist/Album, current play status with live position, and play-state edges from the Y1 in real time, via stock-1.0 `mtkbt` patched to dispatch 1.3+ commands into a JNI-side trampoline chain. Wire target per [`docs/BT-COMPLIANCE.md`](docs/BT-COMPLIANCE.md) §0: AVRCP 1.3 V13 + ESR07 errata. Per-commit motivation, hardware test history, and dead-end paths live in [`docs/INVESTIGATION.md`](docs/INVESTIGATION.md) and `git log`; this CHANGELOG describes the **net delta** from v2.0.0.

### Added
- **AVRCP 1.3 metadata + notification pipeline (`--avrcp` flag).** Patches `mtkbt`, `libextavrcp_jni.so`, `MtkBt.odex`, and `com.innioasis.y1.apk`, plus the `Y1MediaBridge` Android service that publishes Y1's track metadata + play state on disk for the trampoline chain to read. Closes every ICS Table 7 Mandatory row for our claim shape (PASS THROUGH Cat 1 + GetElementAttributes Response) plus every Optional row except 12-17 + 30 (PlayerApplicationSettings — deferred). Per-patch byte-level reference: [`docs/PATCHES.md`](docs/PATCHES.md). Architecture: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). Compliance scorecard against `docs/spec/AVRCP 1.3/AVRCP.ICS.p17.pdf` Table 7: [`docs/BT-COMPLIANCE.md`](docs/BT-COMPLIANCE.md) §2.
- **Discrete PASSTHROUGH PLAY/PAUSE/STOP at the music-app layer (Patch E).** Splits `PlayControllerReceiver`'s short-press `KEY_PLAY → playOrPause()` branch into four discrete arms (toggle / play / pause / stop) so AVRCP PASSTHROUGH 0x44/0x45/0x46 from a peer CT routes to discrete `play(true)` / `pause(0x12, true)` / `stop()` calls instead of toggling. Required by AVRCP 1.3 §4.6.1 + ICS Table 8 (PLAY 0x44 + STOP 0x45 are Mandatory for Cat 1 TGs). Byte-level detail: [`docs/PATCHES.md`](docs/PATCHES.md).
- **Foreground activity propagates unhandled discrete media keys (Patches H + H′ + H″).** `BaseActivity.dispatchKeyEvent` historically returned TRUE for every KeyEvent regardless of whether the activity acted, swallowing `KEYCODE_MEDIA_PLAY` / `_PAUSE` / `_STOP` before they could reach `AudioService` → `PlayControllerReceiver`. Patch H returns `false` for those keycodes (and now `_NEXT` / `_PREVIOUS` per Patch H″) so Patch E's discrete arms run for AVRCP-driven PASSTHROUGH 0x44 / 0x45 / 0x46 / 0x4B / 0x4C. Patch H′ applies the same fix to `BasePlayerActivity.dispatchKeyEvent`, the music-player superclass that overrides `dispatchKeyEvent` itself and bypasses `BaseActivity` entirely. Patch H″ adds a framework-synthetic-repeat filter to both H and H′: when `KeyEvent.getRepeatCount() > 0`, the patched `dispatchKeyEvent` consumes the event silently (returns TRUE without action) so synthetic repeats from `InputDispatcher::synthesizeKeyRepeatLocked` (which fires above the kernel layer and is independent of U1's `EV_REP` NOP) collapse to a single one-shot action per genuine press. Closes the "stuck fast-forwarding" symptom on TV CTs that drop PASSTHROUGH RELEASE under subscribe load. KeyMap variable names are by *role* not direction: `KEY_LEFT=88=MEDIA_PREVIOUS`, `KEY_RIGHT=87=MEDIA_NEXT`, `KEY_PLAY=85=MEDIA_PLAY_PAUSE`. All three patches live entirely inside the music app's APK so `/system/usr/keylayout/AVRCP.kl` stays stock and other foreground apps (e.g. Rockbox) keep spec-correct keycode mapping. Side effect: the music app's hardware NEXT/PREV touch buttons (which emit the same keycodes 87/88) no longer enter long-press FF/RW; they produce one `nextSong()`/`prevSong()` per tap, matching AVRCP 1.3 §4.6.1 op 0x4B/0x4C semantics (FF/RW are op 0x49/0x48, distinct).
- **AVRCP 1.3 Continuation 0x40/0x41 explicit reject (`T_continuation`).** Closes ICS Table 7 rows 31-32 with an explicit dispatch path emitting AV/C NOT_IMPLEMENTED. We never set `packet_type=01` (verified: 2868 PDU 0x20 frames captured, 100% `packet_type=0x00`), so a CT only sends 0x40/0x41 in error from our perspective — zero such PDUs across 43 captures. Byte detail: [`docs/PATCHES.md`](docs/PATCHES.md).
- **AVRCP 1.3 proactive TRACK_REACHED_END / TRACK_REACHED_START on track edge.** T5 emits the §5.4.2 track-edge 3-tuple in spec order — REACHED_END (event 0x03, gated on natural-end flag set by Y1MediaBridge `onTrackDetected()` when the previous track's extrapolated position ≈ duration), TRACK_CHANGED (event 0x02, unconditional), REACHED_START (event 0x04, unconditional). Closes ICS Table 7 rows 25-26 with real-data CHANGED-on-edge. Y1MediaBridge versionCode 17→18; versionName 2.0→2.1.
- **AVRCP 1.3 real BATT_STATUS_CHANGED with CHANGED-on-edge.** Closes ICS Table 7 row 28. Y1MediaBridge bucket-maps Android's `Intent.ACTION_BATTERY_CHANGED` sticky broadcast to the §5.4.2 Tbl 5.35 enum and fires `playstatechanged` on bucket transitions; T9 piggybacks to emit CHANGED on edge. (Stock MtkBt's `BTAvrcpSystemListener.onBatteryStatusChange` chain is dead — overridden with a log-only stub — so reusing `playstatechanged` is the only path that doesn't need new MtkBt.odex code.) Y1MediaBridge versionCode 18→19; versionName 2.1→2.2.
- **AVRCP 1.3 periodic PLAYBACK_POS_CHANGED CHANGED at 1 s cadence while playing.** Closes ICS Table 7 row 27. T9 emits live-extrapolated position via `clock_gettime(CLOCK_BOOTTIME)`, driven by a Y1MediaBridge 1 s `Handler.postDelayed` loop firing `playstatechanged` while playing. Documented spec deviation: the CT-supplied `playback_interval` is ignored; we emit at our 1 s cadence regardless. Spec-permissible — "shall be emitted at this interval" defines a maximum interval ceiling, not a minimum cadence floor. Y1MediaBridge versionCode 19→20; versionName 2.2→2.3.
- **A2DP / AVDTP 1.0 → 1.3 SDP version bumps + best-effort sig 0x0c dispatch alias (V3 + V4 + V5).** V3 / V4 are single-byte LSB bumps in the served A2DP Source SDP record's ProfileDescList (`0xeb9f2`) and ProtocolDescList (`0xeba09`). V5 is a 2-byte edit at the AVDTP signal dispatcher's TBH jump table (file `0xaa834`) that re-points the sig 0x0c (GET_ALL_CAPABILITIES) entry from the BAD_LENGTH stub at `0xab4de` to the existing sig 0x02 (GET_CAPABILITIES) handler at `0xaa924`. V5 is a structural workaround — there's no GET_ALL_CAPABILITIES-specific code path in `mtkbt`; per AVDTP V13 §8.8 the sig 0x02 response is a wire-compatible subset of the sig 0x0c response, sufficient for an SBC-only Source. Closes GAVDP 1.3 ICS Acceptor Table 5 row 9 on paper (the sole Mandatory-row gap surfaced by the §9.10-§9.14 ICS-scoreboard pass). Risk acknowledged: at file `0xaa9fa` the sig 0x02 handler does `strb r2, [r6, 1]` with r2=2; if `[r6+1]` is the response wire sig_id rather than an internal state byte (sibling-handler patterns suggest the latter), a strict peer would see a mismatched response and reject. No current test peer (TV / Bolt) probes Y1 with sig 0x0c so the alias path is unexercised on hardware. Worst case is no worse than stock, which already error-responds to every sig 0x0c with BAD_LENGTH. See [`docs/INVESTIGATION.md`](docs/INVESTIGATION.md) Trace #13c for the dispatcher disassembly + capture history. New `patch_mtkbt.py` `OUTPUT_MD5`: `51a9881d5c5c21b375880cfcf8e23792`.

### Deferred
- **AVRCP 1.3 PlayerApplicationSettings (PDUs 0x11-0x16 + event 0x08).** ICS Table 7 rows 12-17 + 30 — all Optional, with condition C.14 demanding all-or-none for 0x11-0x14. Full deferral rationale + estimated effort + response-builder discovery work in [`docs/BT-COMPLIANCE.md`](docs/BT-COMPLIANCE.md) §1 + §5. Schema bytes 795-799 of `y1-track-info` are reserved for the eventual shuffle/repeat fields.

### Fixed
- **`MediaBridgeService.LogcatMonitor` STOPPED state-code coverage.** Y1 emits state code `'5'` after FF cascades terminate (likely STOPPED). LogcatMonitor previously only recognized `'1'` (PLAYING) and `'3'` (PAUSED) and dropped `'5'`, so `playing_flag` at `y1-track-info[792]` never held 0 (STOPPED) and T6 / T9 reported a stale PAUSED on the wire after a STOPPED transition. Now maps all three; `onStateDetected` refactored from `boolean playing` to `byte avrcpStatus` carrying the §5.4.1 Tbl 5.26 three-valued enum. Y1MediaBridge versionCode 20→21; versionName 2.3→2.4.
- **AVRCP GetElementAttributes attribute cap (240 B, codepoint-safe).** `Y1MediaBridge/MediaBridgeService.java::putUtf8Padded` now caps each string attribute at 240 B before it lands in `y1-track-info`, well below the OEM 511-byte hard cap in `libextavrcp.so:btmtk_avrcp_send_get_element_attributes_rsp` that silently drops over-cap attributes (`[BT][AVRCP][ERR] too large attr_index:%d`). UTF-8 codepoint-safe truncation walks back from the cap to the previous codepoint boundary so a strict CT never receives a partial multi-byte sequence (AVRCP 1.3 §5.3.4 CharacterSet=0x6A). Numeric attributes already fit; string slots (title/artist/album/genre, slot 256) get the cap. Closes [`docs/BT-COMPLIANCE.md`](docs/BT-COMPLIANCE.md) §9.3. Y1MediaBridge versionCode 23→25; versionName 2.6→2.7.
- **AVRCP playback state ↔ AVDTP source state coupling (`patch_libaudio_a2dp.py`, AH1).** New byte patcher: single ARM cond-flip at `libaudio.a2dp.default.so:0x86a8` (`beq 8684` → `b 8684`) makes the call to `a2dp_stop@plt` inside `A2dpAudioStreamOut::standby_l` unreachable. AudioFlinger's silence-timeout standby still completes (release_wake_lock, mStandby = 1) but the AVDTP source stream is left alive across pauses — TV-class CTs no longer cycle their A2DP sink, eliminating the burst-on-resume + playhead-drift symptom. AVDTP 1.3 §8.13 / §8.15 expectation: PAUSED leaves the source stream paused-but-up; SUSPEND is reserved for explicit policy changes. Empirically grounded in capture `/work/logs/dual-tv-20260509-1538`. Wired into `--avrcp`. Closes [`docs/BT-COMPLIANCE.md`](docs/BT-COMPLIANCE.md) §9.2.
- **Reverted earlier §9.2 attempt.** v2.7-v2.8 drove `audioManager.setParameters("A2dpSuspended=true|false")` from `Y1MediaBridge/MediaBridgeService.java::onStateDetected` on every play-state edge. Capture `/work/logs/dual-tv-20260509-1538` showed the AOSP A2DP HAL implements `setSuspended(true)` as a *synchronous* tear-down — `setParameters("A2dpSuspended=true")` calls `a2dp_stop` directly inside the setSuspended path, before any silence-timeout standby fires. The Java approach actively triggered the AVDTP SUSPEND it was trying to prevent. Removed in v2.9; replaced by the HAL byte patch above. Y1MediaBridge versionCode 27→29; versionName 2.8→2.9.
- **`Y1MediaBridge` Android service** (`src/Y1MediaBridge/`) publishing Y1's track metadata + play state on disk via the `y1-track-info` schema for the trampoline chain to read. Reads ID3 tags via `MediaMetadataRetriever` over a FileDescriptor (mediaserver path-resolution sidestep). versionCode 17, versionName 2.0. Schema: [`docs/BT-COMPLIANCE.md`](docs/BT-COMPLIANCE.md) §4. Service detail: [`src/Y1MediaBridge/README.md`](src/Y1MediaBridge/README.md).
- **Diagnostic capture tooling.** `tools/dual-capture.sh` extended with `getevent -lt` + polled `dumpsys input` streams alongside btlog + logcat for ground-truth input-layer correlation. (Closed the kernel-auto-repeat root-cause hunt that landed U1.)
- **`docs/spec/`** (gitignored — Bluetooth SIG copyright). Drop spot for `AVRCP_SPEC_V13.pdf` + `ESR07_ESR_V10.pdf` + `AVRCP.ICS.p17.pdf` + `AVRCP.IXIT.1.6.0.pdf` for citation verification. Source URL pinned in `docs/BT-COMPLIANCE.md` §0.

### Changed
- **Y1MediaBridge versionCode bumped 21→29, versionName 2.4→2.9** for on-device traceability across the 2026-05-09 revert sequence (MediaButton-receiver-deregistration → §9.2 A2dpSuspended Java approach). No functional code change vs. versionCode 21 from the MediaButton work — the bump exists so flashing over the rejected versionCode 22 build leaves a visible marker. Background: 2026-05-08 hardware verification surfaced a chain-break where AVRCP-driven discrete `KEYCODE_MEDIA_PLAY` (0x7e) reached the kernel as `KEY_PLAYCD` but never produced a `Y1Patch: PlayerService.play(Z) entry` log on Sonos / Kia / TV. Working hypothesis was that `MediaBridgeService.setupRemoteControlClient`'s `registerMediaButtonEventReceiver(PlaySongReceiver)` claim was holding the slot in a way that suppressed the actual dispatch. Dropping the registration (versionCode 22) unblocked the broadcast fallback to the music app's `PlayControllerReceiver` but **broke metadata delivery on Kia entirely** — confirming the registered MediaButton client is what MtkBt uses to locate Y1MediaBridge's `IBTAvrcpMusic` Binder. Reverted; the discrete-PLAY-routing chain-break remains an open investigation. See `docs/INVESTIGATION.md` for the per-CT empirical detail.
- **`--avrcp` flag semantics.** Was a legacy SDP byte-patch attempt that regressed PASSTHROUGH without delivering metadata (removed below). Now the working pipeline above (was `--avrcp-min`). Excluded from `--all` because it requires a Y1MediaBridge gradle build first (analogous to `--root` requiring `src/su/` built).
- **GitHub repository renamed `y1-mods` → `koensayr`.** Reverses the v2.0.0 hybrid pattern. GitHub auto-redirects the old URL for existing clones; new clones should use `git clone git@github.com:SeanathanVT/koensayr.git`. Existing clones can update with `git remote set-url origin git@github.com:SeanathanVT/koensayr.git`. Internal `mktemp` prefix in `apply.bash` switched from `y1-fixes.XXXXXX` to `koensayr.XXXXXX` to match.
- **Spec-citation discipline pinned in `docs/BT-COMPLIANCE.md` §0.** Wire target: AVRCP 1.3 V13 + ESR07 errata. F1's BlueAngel-internal version flag is a Java-side dispatcher-unblock value, not a wire-shape upgrade. All section numbers verified against the spec PDFs in `docs/spec/`.

### Removed
- **Legacy SDP byte-patch attempt** (`patch_mtkbt.py` 11-patch B1-B3/C1-C3/A1/D1/E3/E4/E8 set; `patch_libextavrcp_jni.py` C2a/b/C3a/b; `patch_libextavrcp.py` C4). All three regressed PASSTHROUGH without delivering metadata; superseded by the V1/V2/S1/P1 + trampoline-chain pipeline. Diagnosis preserved in [`docs/INVESTIGATION.md`](docs/INVESTIGATION.md).
- **Legacy adbd byte-patch attempts** (`patch_adbd.py` H1/H2/H3 + `patch_bootimg.py` cpio wrapper). Both unwired since v1.7.0; superseded by `src/su/` setuid-root binary since v1.8.0. Full failure-mode diagnosis in [`docs/INVESTIGATION.md`](docs/INVESTIGATION.md) §"adbd Root Patches (H1/H2/H3)".
- **`reference/` directory** (post-patch snapshot of `/system/build.prop` + `/system/etc/bluetooth/*.conf` from one v3.0.2 device). Not consumed by anything in the bash, not stock baselines, and now stale. Restorable from git history.
- **`--avrcp-min` flag** (renamed to `--avrcp`) and the `--avrcp` ↔ `--avrcp-min` mutual-exclusion check.
...
- `INVESTIGATION.md` → `docs/INVESTIGATION.md`. Long-form investigation history now lives alongside `docs/ARCHITECTURE.md` / `docs/PATCHES.md` / `docs/BT-COMPLIANCE.md`. `README.md` and `CHANGELOG.md` stay at the repo root by GitHub-UI convention.
...
- **README polish for v2.0.0.** Overview's misleading "Bluetooth metadata" lead bullet (which described the now-known-broken `--avrcp` flow) reframed; the Artist→Album / generic "APK patching" bullets consolidated; an "AVRCP investigation tooling" bullet added that points at [Status](README.md#status). Redundant Scripts section dropped — its per-patcher detail already lives in [`src/patches/README.md`](src/patches/README.md) (table) and [`docs/PATCHES.md`](docs/PATCHES.md) (byte-level). Layout section gains `tools/` and `staging/` rows. "Manual patcher invocation" subsection collapsed to a one-liner pointer. "Verified against" trimmed (overlapped with the firmware manifest above it). 156 → 135 lines.
...
### Removed
- **Four failed in-tree experiment scripts** (all confirmed disproven; testing-only by design). Their names, byte-level details, and disproof traces are recorded in `INVESTIGATION.md` "Conclusion (2026-05-04)" and the git history. Diagnostic tooling (`@btlog` tap + dual-capture + parser + post-root probe) is unaffected and remains in-tree for the proxy-work prep.
...
- 2026-05-02 — Remove E5 / E7a / E7b from `patch_mtkbt.py` (empirically inert across the CT test matrix).
...
- 2026-04-30 — Regression analysis confirms three `AttrID=0x0009` (ProfileDescList) entries; restored and upgraded all SDP-record version slots.
...
- New `patch_libextavrcp.py` (libextavrcp.so SDP version constant patch — part of the legacy byte-patch attempt later removed). Renamed `patch_so.py` → `patch_libextavrcp_jni.py`.
...
- `patch_mtkbt.py`, `patch_odex.py`, `patch_so.py` — initial three BT binary patchers (legacy SDP-shape byte-patch attempt; later removed).
