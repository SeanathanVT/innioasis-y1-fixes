...
This release lands the **AVRCP 1.3 metadata pipeline**: a peer Bluetooth Controller (car head unit, TV, smart speaker, etc.) now sees Title/Artist/Album, current play status with live position, and play-state edges from the Y1 in real time, via stock-1.0 `mtkbt` patched to dispatch 1.3+ commands into a JNI-side trampoline chain. Wire target per [`docs/AVRCP13-COMPLIANCE-PLAN.md`](docs/AVRCP13-COMPLIANCE-PLAN.md) §0: AVRCP 1.3 V13 + ESR07 errata. Per-iter motivation, hardware test history, and dead-end paths live in [`docs/INVESTIGATION.md`](docs/INVESTIGATION.md) and `git log`; this CHANGELOG describes the **net delta** from v2.0.0.

### Added
- **AVRCP 1.3 metadata + notification pipeline (`--avrcp` flag).** Patches `mtkbt` (V1/V2/S1/P1: SDP shape + force-PASSTHROUGH-emit op_code dispatch), `libextavrcp_jni.so` (R1 redirect + T1/T2/extended_T2/T4/T5/T_charset/T_battery/T6/T8/T9 trampoline chain in LOAD #1 padding extension + U1 kernel-auto-repeat NOP), `MtkBt.odex` (F1/F2 + two cardinality NOPs to wake the JNI native callbacks), and `com.innioasis.y1.apk` (Patch E). Ships the `Y1MediaBridge` Android service that publishes Y1's track metadata + play state on disk for the trampoline chain to read. Implements every **ICS Table 7 mandatory row** for our claim shape (PASS THROUGH Cat 1 + GetElementAttributes Response): GetCapabilities (PDU 0x10), GetElementAttributes (0x20 — **all 7 §5.3.4 attributes: Title/Artist/Album/TrackNumber/TotalNumberOfTracks/Genre/PlayingTime — packed in a single response**), GetPlayStatus (0x30, with `clock_gettime(CLOCK_BOOTTIME)`-based live position extrapolation), RegisterNotification (0x31) with INTERIM coverage of all 7 events 0x01–0x07 and proactive CHANGED-on-edge for 0x01 PLAYBACK_STATUS_CHANGED + 0x02 TRACK_CHANGED. Plus the optional CT→TG informational PDUs InformDisplayableCharacterSet (0x17) and InformBatteryStatusOfCT (0x18). Per-patch byte-level reference in [`docs/PATCHES.md`](docs/PATCHES.md); architecture in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md); compliance scorecard against `docs/spec/AVRCP.ICS.p17.pdf` Table 7 in [`docs/AVRCP13-COMPLIANCE-PLAN.md`](docs/AVRCP13-COMPLIANCE-PLAN.md) §2.
- **Discrete PASSTHROUGH PLAY/PAUSE/STOP at the music-app layer (Patch E).** AVRCP 1.3 §4.6.1 (PASS THROUGH command, op codes defined in AV/C Panel Subunit Specification ref [2]) + ICS Table 8 (Cat 1 op_id status) require a TG advertising Cat 1 to honor PLAY (0x44, M) and STOP (0x45, M) as discrete state transitions. Patch E in `patch_y1_apk.py` splits `PlayControllerReceiver`'s short-press `KEY_PLAY → playOrPause()` branch into four discrete arms: legacy `KEY_PLAY` (85, `MEDIA_PLAY_PAUSE`) → `playOrPause()` (toggle); `KEYCODE_MEDIA_PLAY` (126) → `play(true)`; `KEYCODE_MEDIA_PAUSE` (127) → `pause(0x12, true)`; `KEYCODE_MEDIA_STOP` (86) → `stop()`. The `play(Z)V` boolean must be `true` so the underlying `Static.setPlayValue()` singleton edge fires alongside the player's `start()` — this is what propagates the resume to the rest of the music app (UI, RemoteControlClient, AudioFocus). Without it, the player resumes but other components don't see the edge and either fight it back to paused or never reflect the change.
- **Foreground activity propagates unhandled discrete media keys (Patch H).** `BaseActivity.dispatchKeyEvent` (every Activity in the music app extends `BaseActivity` → `AppCompatActivity`) historically returned TRUE unconditionally, so every KeyEvent reaching the foreground activity was consumed regardless of whether the activity acted. For the device's hardware scroll-wheel keys (KeyMap-defined LEFT/RIGHT/UP/DOWN/MENU/PLAY/ENTER) the activity dispatches directly to PlayerService and consumption is correct, but for `KEYCODE_MEDIA_PLAY` (`0x7e`) / `KEYCODE_MEDIA_PAUSE` (`0x7f`) / `KEYCODE_MEDIA_STOP` (`0x56`) — none of which appear in the KeyMap — control fell through every if-eq check, returned TRUE, and the events were silently swallowed. They never reached `PhoneFallbackEventHandler.handleMediaKeyEvent` → `AudioService` → the `ACTION_MEDIA_BUTTON` broadcast → `PlayControllerReceiver`, so Patch E's discrete play/pause/stop arms never ran for AVRCP-driven PASSTHROUGH 0x44/0x45/0x46. Patch H inserts an early `return false` for those three keycodes immediately after `getKeyCode()` in `dispatchKeyEvent`, restoring spec-correct end-to-end routing. The patch lives entirely inside the music app's APK; `/system/usr/keylayout/AVRCP.kl` stays stock, so other foreground apps installable on the device (e.g. Rockbox) — which do not inherit from `com.innioasis.y1.base.BaseActivity` — see the spec-correct keycode mapping.
- **AVRCP 1.3 Phase D — explicit Continuation 0x40/0x41 reject (`T_continuation`).** New trampoline branched from T4's pre-check when the inbound PDU is 0x40 RequestContinuingResponse or 0x41 AbortContinuingResponse. Routes both to the existing UNKNOW_INDICATION path that emits AV/C NOT_IMPLEMENTED via msg=520. `get_element_attributes_rsp` never sets the start-of-fragmentation flag (verified: 2868 PDU 0x20 frames in a single TV capture, 100% `packet_type=0x00`); mtkbt fragments below at the AVCTP layer transparently to AVRCP. Across all 43 captures, zero 0x40/0x41 PDUs from any CT in the test matrix. AVRCP 1.3 §6.15.2 specifies INVALID_PARAMETER (status 0x05) as the strict-spec reject; NOT_IMPLEMENTED is a different but spec-acceptable AV/C reject and is functionally indistinguishable to the CT (both abandon the continuation flow). Closes ICS Table 7 rows 31-32 with an explicit dispatch path; preserves the spec-acceptable wire shape.
- **AVRCP 1.3 Phase F1 — proactive TRACK_REACHED_END / TRACK_REACHED_START on track edge.** T5 (proactive trampoline entered via `b.w T5` from the patched `notificationTrackChangedNative`) now emits the AVRCP 1.3 §5.4.2 track-edge 3-tuple in spec-defined order on every track edge: `reg_notievent_reached_end_rsp` (event 0x03, Tbl 5.31) gated on the natural-end flag, `track_changed_rsp` (event 0x02, Tbl 5.30) unconditional, `reg_notievent_reached_start_rsp` (event 0x04, Tbl 5.32) unconditional. The 0x03 gate ships strict spec semantic ("Notify when reached the end of the track of the playing element" — natural-end-only, not skip-driven): Y1MediaBridge `onTrackDetected()` compares the previous track's extrapolated position (`computePosition()`) against `mCurrentDuration` at the moment of the track edge; within `[-1000ms..+2000ms]` of duration counts as natural end. The result is written to `y1-track-info[793]` as a u8 before the metachanged broadcast fires; T5 reads it from the file buffer. T5's stack frame grew from 24 B to 816 B (16 state + 800 file_buf) so it can read offset 793. Closes ICS Table 7 rows 25-26 with real-data CHANGED-on-edge alongside the existing T8 INTERIM coverage. Y1MediaBridge versionCode 17→18; versionName 2.0→2.1.
- **AVRCP 1.3 Phase F2 — real BATT_STATUS_CHANGED with CHANGED-on-edge.** Closes ICS Table 7 row 28. `Y1MediaBridge` gains a `BroadcastReceiver` for Android's `Intent.ACTION_BATTERY_CHANGED` sticky broadcast. `handleBatteryIntent()` bucket-maps `EXTRA_LEVEL` / `EXTRA_PLUGGED` / `EXTRA_STATUS` to the AVRCP §5.4.2 Tbl 5.35 enum (`STATUS_FULL → 4 FULL_CHARGE; plugged != 0 → 3 EXTERNAL; pct ≤ 15 → 2 CRITICAL; pct ≤ 30 → 1 WARNING; else → 0 NORMAL`). Cold-boot reads the sticky broadcast via the `registerReceiver` return value. On bucket transitions only, `mCurrentBatteryStatus` updates, `writeTrackInfoFile()` persists the new value to `y1-track-info[794]`, and a `playstatechanged` broadcast fires to wake T9. T8's existing event-0x06 INTERIM arm now reads `y1-track-info[794]` instead of returning canned `0x00 NORMAL`; T9 extends to also compare `file[794]` vs `y1-trampoline-state[10]` (`last_battery_status`, was pad) and emit `reg_notievent_battery_status_changed_rsp` CHANGED on edge alongside the existing play-status path. Trigger discovery: stock MtkBt's battery dispatch chain through `BTAvrcpSystemListener.onBatteryStatusChange` is dead — `BTAvrcpMusicAdapter$2` overrides it with a `Log.i(...)` stub — and `IBTAvrcpMusic` AIDL exposes no battery-notify surface, so reusing `playstatechanged` as the trigger is the only spec-compliant path that doesn't require new MtkBt.odex code. No MtkBt.odex change: existing cardinality NOP at sswitch_18a / `0x3c4fe` already wakes T9 on every `playstatechanged`. Y1MediaBridge versionCode 18→19; versionName 2.1→2.2.
- **`Y1MediaBridge` Android service** (`src/Y1MediaBridge/`) publishing Y1's track metadata + play state on disk via the `y1-track-info` schema (8-byte synthetic track_id + Title/Artist/Album in 256-byte UTF-8 fields + duration_ms / position_at_state_change_ms / state_change_time_sec / playing_flag + GetElementAttributes attrs 4-7 as pre-formatted UTF-8 strings: TrackNumber / TotalNumberOfTracks / PlayingTime / Genre, total 1104 B). Reads ID3 tags directly via `MediaMetadataRetriever` over a FileDescriptor (uid-1013 mediaserver-side path-resolution sidestep) with `WRITE_MEDIA_STORAGE` permission for `media_rw` group access. Genre is sourced from `MediaStore.Audio.Genres` (or `METADATA_KEY_GENRE` on the direct-tag fallback path); track number from `MediaStore.Audio.Media.TRACK % 1000` (or parsed from `METADATA_KEY_CD_TRACK_NUMBER` "n/total"); total from a `count(*) WHERE ALBUM_ID=?` query (or from the "/total" half of CD_TRACK_NUMBER). Reorders broadcast emission to write file before `metachanged` dispatch (avoids T5 cross-process race) and writes `playing_flag` before `playstatechanged` (so T6 GetPlayStatus polls + T9 CHANGED-on-edge see fresh state). versionCode 17, versionName 2.0.
...
- **`docs/spec/`** (gitignored — Bluetooth SIG copyright). Drop spot for `AVRCP_SPEC_V13.pdf` + `ESR07_ESR_V10.pdf` + `AVRCP.ICS.p17.pdf` + `AVRCP.IXIT.1.6.0.pdf` for citation verification. Source URL pinned in COMPLIANCE-PLAN §0.
...
- **Spec-citation discipline pinned in `docs/AVRCP13-COMPLIANCE-PLAN.md` §0.** Wire target: AVRCP 1.3 V13 + ESR07 errata. F1's BlueAngel-internal version flag is a Java-side dispatcher-unblock value, not a wire-shape upgrade. All section numbers verified against the spec PDFs in `docs/spec/`.
...
- `INVESTIGATION.md` → `docs/INVESTIGATION.md`. Long-form investigation history now lives alongside `docs/ARCHITECTURE.md` / `docs/PATCHES.md` / `docs/AVRCP13-COMPLIANCE-PLAN.md`. `README.md` and `CHANGELOG.md` stay at the repo root by GitHub-UI convention.
