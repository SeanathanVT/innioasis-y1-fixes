...
### Changed
- **`--avrcp` flag semantics.** Was the legacy AVRCP 1.4 byte-patch attempt (regressed PASSTHROUGH without delivering metadata; removed below). Now the working pipeline above (was `--avrcp-min`). Excluded from `--all` because it requires a Y1MediaBridge gradle build first (analogous to `--root` requiring `src/su/` built).
...
- **Spec-citation discipline pinned in `docs/AVRCP13-COMPLIANCE-PLAN.md` §0.** Wire target: AVRCP 1.3 V13 + ESR07 errata (1.3 deprecated 2023-02-01, withdrawn 2027-02-01 per ICS §1.2 Table 2b — does not block shipping; we're patching a 2012 firmware). F1's BlueAngel-internal "1.4" version flag is a dispatcher-unblock flag, not a wire-shape upgrade; we ship zero 1.4-only PDUs (no SetAddressedPlayer 0x60, no SetAbsoluteVolume 0x50, no Browsing Channel 0x70..0x77). All section numbers verified against the spec PDFs in `docs/spec/`. Drift→verified citation mapping table at the end of §0.

### Removed
- **Legacy AVRCP 1.4 byte-patch attempt** (`patch_mtkbt.py` 11-patch B1-B3/C1-C3/A1/D1/E3/E4/E8 set; `patch_libextavrcp_jni.py` C2a/b/C3a/b; `patch_libextavrcp.py` C4). All three caused or relied on the broken AVRCP 1.4 wire-protocol attempt and have been superseded by the V1/V2/S1/P1 + trampoline-chain pipeline. Diagnosis preserved in [`docs/INVESTIGATION.md`](docs/INVESTIGATION.md).
