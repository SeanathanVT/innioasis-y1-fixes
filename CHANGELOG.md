...
- **`--help` handling added to four scripts that previously did the wrong thing.** `tools/setup.sh --help` was running the full setup (cloning MTKClient ~97 MB + creating venvs); `tools/install-android-sdk.sh --help` was starting a ~165 MB SDK download; `tools/release.sh --help` was failing with "ERROR: '--help' is not a strict X.Y.Z semver" (fail-fast but unhelpful); `src/patches/patch_y1_apk.py --help` was treating `--help` as the APK path and exiting with `ERROR: '--help' not found.`. All four now check for `-h|--help` upfront and print actual usage info.
