...
- **`GetElementAttributes` §5.3.4 spec compliance.** T4 now emits all 8 known attribute IDs (the seven AVRCP 1.3 attributes 0x01-0x07 plus the AVRCP 1.6 §5.14.1 Default Cover Art slot 0x08, attribute id assigned in §26 Table 26.1 per ESR09 E6073), and the new `patch_libextavrcp.py` E1 removes the stock `libextavrcp.so` "ignore empty attrib" drop so unsupported attributes (including 0x08, which Y1 doesn't implement transfer for) emit with `AttributeValueLength=0` per §5.3.4. Strict CTs that gate their metadata-pane render on receiving every requested attribute back — even with zero length — now receive a §5.3.4-compliant response shape instead of the silently-truncated one they got before.
