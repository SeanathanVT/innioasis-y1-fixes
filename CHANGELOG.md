...
### Added
- **`tools/experiment-features-01.sh`** + **`tools/patch_mtkbt_features_01.py`** — temporary in-tree experiment (testing-only). Tests the smallest possible feature-bit change: overrides E3/E4 SupportedFeatures from `0x33` to `0x01` (Cat1-only, matching Pixel 4's AVRCP 1.3 features bitmask) while keeping AVRCP 1.4 advertised everywhere else. Hypothesis: Y1's bug isn't the version inflation per se — it's that we *also* claim feature bits whose handlers mtkbt doesn't have. Trimming features back to bare-Cat1 may let Sonos engage with whatever 1.3-class command handling mtkbt actually has. Single-byte tweak each at E3/E4. Same transactional bash-wrapper pattern as the previous experiments. Output binary MD5 will differ; verify-only smoke confirmed both sites land at `0x01`.
- **`tools/experiment-pixel-1-3.sh`** + **`tools/patch_mtkbt_pixel_1_3.py`** + **`tools/patch_mtkbt_odex_pixel_1_3.py`** — temporary in-tree experiment (testing-only). Full byte-for-byte mimic of Pixel 4's AVRCP TG record at AVRCP 1.3 (verified working with Sonos via user test 2026-05-04: title/artist/album + play/pause both work). Drops AVCTP advertisement 1.3 → 1.2 (B1/B2/B3 0x03 → 0x02), AVRCP advertisement 1.4 → 1.3 (C1/C2/C3 0x04 → 0x03), SupportedFeatures `0x33` → `0x01` (E3/E4), reverts A1's runtime MOVW r7 immediate `0x0401` → `0x0301`, and reverts F1 in `MtkBt.odex` so `getPreferVersion()` returns 10 (= AVRCP 1.3 in BlueAngel's enum) instead of 14 (= 1.4). All other patches (D1, E8, F2, C2a/b, C3a/b, C4) unchanged. Hypothesis: mtkbt may have latent AVRCP 1.3 command handlers that activate when we advertise 1.3 with a coherent shape; the brief's standard --avrcp inflates version to 1.4 but mtkbt may be internally a 1.3-class implementation we've been mis-advertising. Bash wrapper shadows BOTH `patch_mtkbt.py` and `patch_mtkbt_odex.py` (renames each to `*_orig.py`, drops the experiment patcher in place, restores both on EXIT trap). Output binary MD5: `eb7a47a99b6b93d2ac965c56f6ad4730`. Verify-only smoke confirmed all 9 byte-level sites (8 single-byte + the 4-byte A1 sequence) land correctly.
- **`tools/experiment-pixel-shape.sh`** + **`tools/patch_mtkbt_pixel_shape.py`** — temporary in-tree experiment (testing-only; not wired into any default flag). Re-flashes mtkbt with three SDP-record-shape changes that match the AVRCP TG record a Pixel 4 advertises (the working reference confirmed via Pixel 4 ↔ Sonos Roam metadata transfer): bumps AVCTP advertisement 1.3 → 1.4 (one-byte tweak each at the B1/B2/B3 sites), bumps AVRCP advertisement 1.4 → 1.5 (one-byte tweak each at C1/C2/C3), and overrides SupportedFeatures `0x0033 → 0x00d1` at E3/E4 (changing Cat1+Cat2+PAS+GroupNav to Cat1+PAS+Browsing+MultiPlayer — only Cat1+PAS in common; the brief's previous "AOSP Bluedroid 1.4 baseline" claim turns out to predate Android 10's actual baseline). Implemented as a transactional bash wrapper that renames `src/patches/patch_mtkbt.py` → `patch_mtkbt_orig.py`, drops the experiment patcher in its place, runs the standard `--avrcp --bluetooth` flow, and restores on EXIT (trap). The patcher imports `patch_mtkbt_orig` and mutates only the targeted entries by offset, so it picks up any future changes to the base patch list. Reverting after a flash is a normal `--avrcp --bluetooth` re-flash. Output binary MD5: `2a11385ce331d4eb60f794811db9d5ff`. Hypothesis under test: 1.4-aware CTs (Sonos, car) decline to issue AVRCP COMMANDs against a TG whose SDP record advertises 1.4 capability without the modern feature bits Pixel sets. Reading-B test follow-up to the bridge-disable test (which proved the Y1MediaBridge presence is not actively interfering — the version push comes from F1 and the binary patches, not the bridge).
...
### Changed
- README `Status` section updated for the post-root investigation pivot. The `result:4096` lead is documented as a phantom — the same value is emitted at `ACTIVATE_CNF` time 3 ms after the JNI sends `ACTIVATE_REQ`, before any peer is involved; it's mtkbt's standard "request acknowledged" status code, not a peer-feedback or "feature degraded" indicator. Browsing-bit hypothesis (`SupportedFeatures 0x0033 → 0x0073`) tested and disproven — landed on the wire but didn't change peer behaviour. Active investigation reframed around the SDP-record A/B between Pixel 4 (working TG) and Y1 (broken TG) using `sdptool browse` from a Linux laptop. Adjacent functional symptom documented: CT→Y1 `PASS_THROUGH` play/pause is broken in the car, suggesting AVRCP COMMAND processing is broken end-to-end on the Y1 side, not just notification-cosmetic.
- README `Layout` and `Scripts` sections cross-link the new `src/btlog-dump/` tree.
- New top-level `Diagnostics` README section documenting the `@btlog` tap workflow and the post-root probe.
