...
- **`Y1MediaBridge` Android service** (`src/Y1MediaBridge/`) publishing Y1's track metadata + play state on disk via the `y1-track-info` schema (8-byte synthetic track_id + Title/Artist/Album in 256-byte UTF-8 fields + duration_ms / position_at_state_change_ms / state_change_time_sec / playing_flag). Reads ID3 tags directly via `MediaMetadataRetriever` over a FileDescriptor (uid-1013 mediaserver-side path-resolution sidestep) with `WRITE_MEDIA_STORAGE` permission for `media_rw` group access. Reorders broadcast emission to write file before `metachanged` dispatch (avoids T5 cross-process race) and writes `playing_flag` before `playstatechanged` (so T6 GetPlayStatus polls + T9 CHANGED-on-edge see fresh state). versionCode 17, versionName 2.0.
