...
This release lands the **AVRCP 1.3 metadata + control pipeline**: a peer Bluetooth Controller (car head unit, TV, smart speaker) now sees full track metadata, live play status, and play-state changes from the Y1, and can change Repeat / Shuffle from its own UI. Reference docs: [`docs/BT-COMPLIANCE.md`](docs/BT-COMPLIANCE.md), [`docs/PATCHES.md`](docs/PATCHES.md), [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). Investigation history and dead-end paths: [`docs/INVESTIGATION.md`](docs/INVESTIGATION.md).

### Added
- **AVRCP 1.3 metadata over Bluetooth.** Peer CT sees Title / Artist / Album / Genre / TrackNumber / TotalNumberOfTracks / PlayingTime, with strict-CT-friendly UTF-8 codepoint-safe truncation at 240 B per attribute.
- **Live play status with millisecond-precision position.** PLAYING / PAUSED / STOPPED edges land on the wire as soon as the Y1 transitions; position updates at 1 s cadence while playing, with track-edge anchoring keeping CT-side and Y1-side playheads in lockstep across pause/resume. Position freezes at duration on natural end-of-track and resets cleanly on the next track's prepare.
- **Track-change notifications** with proactive TRACK_REACHED_END (gated on natural end-of-track) + TRACK_REACHED_START on every track edge.
- **Battery-status notifications** bucket-mapped from Android's system battery broadcasts, with CHANGED-on-edge semantics.
- **Seek hook** — the music app's in-UI seek bar propagates to the CT's playhead display immediately, without waiting for the next 1 s tick.
- **PlayerApplicationSettings** (Repeat / Shuffle) — bidirectional. CT can change either from its own UI and the Y1 reflects the change; Y1-UI changes push to the CT as wire-CHANGED notifications. The Y1's own Now Playing screen also reflects CT-driven Repeat / Shuffle changes in real time, without needing to navigate away and back.
- **Discrete PASSTHROUGH** (PLAY / PAUSE / STOP / NEXT / PREVIOUS) routes to discrete music-app actions instead of the stock toggle behavior, eliminating the "stuck fast-forwarding" symptom on TV-class CTs that drop PASSTHROUGH RELEASE under subscribe load. PASSTHROUGH PLAY against an already-playing target routes to a pause-toggle, so CTs that map their Pause button to PLAY get correct behavior.
...
- **Per-subscription notification gating** per AVRCP 1.3 §6.7.1. Each CT `RegisterNotification` registration yields exactly one INTERIM + one CHANGED, then waits for the CT to re-register. Matches iPhone/Pixel-class TG semantics; strict CTs no longer ignore "excess" CHANGEDs that would have arrived between their re-registrations.
- **`Y1Bridge` Android service** (`src/Y1Bridge/`, package `com.koensayr.y1.bridge`) — Binder host that satisfies MtkBt's `bindService(com.android.music.MediaPlaybackService)`. Answers synchronous `IBTAvrcpMusic` queries (play status, position, duration, audio id, track/artist/album names, repeat/shuffle modes) by reading the music-app-owned state file directly, so MtkBt's Java mirror always reflects current state. Built via Gradle. The bridge APK only declares the intent-filter that the music APK can't (sharedUserId / platform-key constraint).
- **`GetElementAttributes` AVRCP 1.3 §6.6.1 + §5.3.4 spec compliance.** T4 now reads the CT's inbound `NumAttributes` byte + `AttributeID[N]` array and emits exactly the requested attributes in the requested order (per §6.6.1 Table 6.26). For unsupported attribute IDs (anything outside the canonical 1.3 set 0x01-0x07), Y1 emits the attribute header with `AttributeValueLength=0` per §5.3.4 ("for attributes not supported by the TG, this field shall be sent with 0 length data"). When the CT sets `NumAttributes=0`, Y1 falls back to "return all" — all seven 1.3 attributes in canonical order. The new `patch_libextavrcp.py` E1 removes the stock `libextavrcp.so` response-builder's "ignore empty attrib" drop so the §5.3.4 zero-length emission actually reaches the wire. Strict CTs that gate their metadata-pane render on receiving every requested attribute back — even those they don't actually know about — now get a §5.3.4-compliant response shape.
...
- **Legacy adbd byte-patch attempts.** Superseded by the `src/su/` setuid-root binary.
- **`reference/` directory** (stale post-patch device snapshot). Restorable from git history if needed.
