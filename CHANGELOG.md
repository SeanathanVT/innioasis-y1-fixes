...
### Fixed
- **AVRCP-driven PLAY / PAUSE / STOP no longer drops on Sonos / Kia / TV.** `MediaBridgeService.setupRemoteControlClient` previously called `AudioManager.registerMediaButtonEventReceiver(PlaySongReceiver)`, claiming the system's single-receiver MediaButton slot. Postflash captures (2026-05-08) showed the slot was held by neither us nor the music app at AVRCP-press time — `ACTION_MEDIA_BUTTON` dispatch fell into a hole and discrete `KEYCODE_MEDIA_PLAY` (0x7e) / `_PAUSE` (0x7f) / `_STOP` (0x56) never reached `PlayControllerReceiver`. Removed the `registerMediaButtonEventReceiver` call (and the matching unregister, plus PlaySongReceiver's manifest filter for `ACTION_MEDIA_BUTTON` and its dead forwarding path). The RCC PendingIntent now points directly at `com.innioasis.y1/PlayControllerReceiver` so lock-screen controls still work; AudioService's ordered-broadcast fallback delivers AVRCP keys to PlayControllerReceiver (priority MAX_VALUE, declared in the music app's stock manifest) where Patch E's discrete arms route them. Y1MediaBridge versionCode 21→22; versionName 2.4→2.5.
