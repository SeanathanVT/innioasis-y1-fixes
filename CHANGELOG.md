...
- **Default Cover Art attribute** (AVRCP 1.6 §5.14.1; attribute id `0x08` per §26 Table 26.1 / ESR09 E6073) in the `GetElementAttributes` response. Currently emits an empty handle (per §5.3.4 zero-length convention = "attribute not available"); a follow-up will wire the AVRCP Cover Art OBEX responder + populate the per-track 7-character BIP Image Handle so CTs that gate their metadata-pane render on a non-empty cover-art handle no longer present an empty pane.
