...
- **AVRCP GetElementAttributes attribute cap (240 B, codepoint-safe).** `Y1MediaBridge/MediaBridgeService.java::putUtf8Padded` now caps each string attribute at 240 B before it lands in `y1-track-info`, well below the OEM 511-byte hard cap in `libextavrcp.so:btmtk_avrcp_send_get_element_attributes_rsp` that silently drops over-cap attributes (`[BT][AVRCP][ERR] too large attr_index:%d`). UTF-8 codepoint-safe truncation walks back from the cap to the previous codepoint boundary so a strict CT never receives a partial multi-byte sequence (AVRCP 1.3 §5.3.4 CharacterSet=0x6A). Numeric attributes already fit; string slots (title/artist/album/genre, slot 256) get the cap. Closes [`docs/AVRCP13-COMPLIANCE.md`](docs/AVRCP13-COMPLIANCE.md) §9.3. Y1MediaBridge versionCode 23→25; versionName 2.6→2.7.
- **AVRCP playback state ↔ AVDTP source state coupling.** `Y1MediaBridge/MediaBridgeService.java::onStateDetected` now issues `audioManager.setParameters("A2dpSuspended=true")` on PLAYING→PAUSED edges and `=false` on PAUSED→PLAYING / STOPPED edges. Without this hook, AudioFlinger's silence-timeout (~3 s) hit `libaudio.a2dp.default.so::standby_l` with `mSuspended_param == 0` and called `a2dp_stop` → AVDTP SUSPEND on the wire on every pause-of-≥3 s. TV-class CTs that aggressively power-cycle their A2DP sink would then close+reopen the stream once per cycle, causing audible burst-on-resume (encoder/decoder buffer drain+refill at >1× speed) and momentary playhead-vs-AVRCP-position drift. Empirical confirmation: 8 such cycles in a single 3-min capture (`/work/logs/dual-tv-20260509-1410`), each correlated with `Audio hardware entering standby ... suspend count 0`. The fix sets `mSuspended_param != 0` so `standby_l` skips `a2dp_stop` while paused; the AVDTP stream stays in STREAMING, no renegotiation on resume. Pure Java change (no native trampoline / ELF / mtkbt-side surgery). Symbol-scope verified: `A2dpSuspended` is referenced only by `libaudio.a2dp.default.so` — absent from `mtkbt`, `libmtka2dp.so`, `libmtkbtextadpa2dp.so` — so no MtkBt / BlueAngel side-effects. Supersedes the original [`docs/AVRCP13-COMPLIANCE.md`](docs/AVRCP13-COMPLIANCE.md) §9.2 plan (which proposed a `dlsym` ELF graft into `libextavrcp_jni.so` to reach `btmtk_a2dp_pause_immediately`); the AOSP-HAL parameter is one layer up and achieves the same effect through AudioFlinger's existing flow. Y1MediaBridge versionCode 25→27; versionName 2.7→2.8.
- **`Y1MediaBridge` Android service** (`src/Y1MediaBridge/`) publishing Y1's track metadata + play state on disk via the `y1-track-info` schema for the trampoline chain to read. Reads ID3 tags via `MediaMetadataRetriever` over a FileDescriptor (mediaserver path-resolution sidestep). versionCode 17, versionName 2.0. Schema: [`docs/AVRCP13-COMPLIANCE.md`](docs/AVRCP13-COMPLIANCE.md) §4. Service detail: [`src/Y1MediaBridge/README.md`](src/Y1MediaBridge/README.md).
...
- **Spec-citation discipline pinned in `docs/AVRCP13-COMPLIANCE.md` §0.** Wire target: AVRCP 1.3 V13 + ESR07 errata. F1's BlueAngel-internal version flag is a Java-side dispatcher-unblock value, not a wire-shape upgrade. All section numbers verified against the spec PDFs in `docs/spec/`.
...
- `INVESTIGATION.md` → `docs/INVESTIGATION.md`. Long-form investigation history now lives alongside `docs/ARCHITECTURE.md` / `docs/PATCHES.md` / `docs/AVRCP13-COMPLIANCE.md`. `README.md` and `CHANGELOG.md` stay at the repo root by GitHub-UI convention.
