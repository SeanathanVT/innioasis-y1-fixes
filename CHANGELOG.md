...
- **Foreground activity propagates unhandled discrete media keys (Patch H).** `BaseActivity.dispatchKeyEvent` historically returned TRUE for every KeyEvent regardless of whether the activity acted, swallowing `KEYCODE_MEDIA_PLAY` / `_PAUSE` / `_STOP` before they could reach `AudioService` → `PlayControllerReceiver`. Patch H returns `false` for those three keycodes so Patch E's discrete arms run for AVRCP-driven PASSTHROUGH 0x44/0x45/0x46. Lives entirely inside the music app's APK so `/system/usr/keylayout/AVRCP.kl` stays stock and other foreground apps (e.g. Rockbox) keep spec-correct keycode mapping.
