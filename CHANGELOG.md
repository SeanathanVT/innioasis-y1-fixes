...
- `mtkbt`: M1a / M1b retarget the RegisterNotification response ctype discriminator in `fcn.0x121d8` (PDU 0x31 response packetFrame builder) so the wire ctype matches the reasonCode the JNI trampoline passes. Stock mtkbt loaded the INTERIM/CHANGED discriminator from ctxt[8] and compared it against 1 — but the JNI's `btmtk_avrcp_send_reg_notievent_*_rsp` helpers in `libextavrcp.so` actually marshal the reasonCode (`0x0F` INTERIM / `0x0D` CHANGED) into ctxt[7], leaving ctxt[8] zero from memset. The cmp always missed, dispatch always landed on the CHANGED branch (`movs r1, 0x0D` at `0x12244`), and wire ctype was `0x0D` for every RegNotif response regardless of what the trampoline requested. M1a flips the load offset from 8 to 7 (`21 7a` → `e1 79`); M1b flips the cmp constant from 1 to `0x0F` (`01 29` → `0f 29`). After both: `ctxt[7] == 0x0F` → wire ctype `0x0F` INTERIM (T2 / T8 first-response arms), `ctxt[7] != 0x0F` → wire ctype `0x0D` CHANGED (T5 / T9 edge emits). Spec-compliant per AVRCP 1.3 §6.7.1 and matches the Pixel-as-TG btsnoop pattern. Earlier "M1 / M1b / M1c" patches in fn `0x379e0` and an "M1d" in fn `0x396d0` were confirmed dead code (different functions, different work struct offset) and have been removed — see `docs/INVESTIGATION.md` Trace #37 for the full end-to-end byte chain.
...
- TRACK_CHANGED `Identifier` payload changed from `0xFFFFFFFFFFFFFFFF` ("no media currently selected" per AVRCP 1.3 §5.4.2 Tbl 5.30 + ESR07 §2.2) to `0x0000000000000000` ("currently playing media is selected", AVRCP 1.4+ SELECTED). Mirrors what Pixel-as-TG sends. The previous sentinel told strict 1.4+ CTs there was no track playing, suppressing metadata-pane render. Y1 always has a track loaded when a CT is connected, so SELECTED is the honest signal. The `y1-trampoline-state[0..7]` field still holds the real per-track UID internally so T4 / T5 detect edges and emit CHANGED proactively — only the on-wire payload is constant.
