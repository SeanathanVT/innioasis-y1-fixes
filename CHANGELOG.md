...
- TRACK_CHANGED `Identifier` payload sent as `0x0000000000000000` ("currently playing media is selected", AVRCP 1.4+ SELECTED). Mirrors what Pixel-as-TG sends to the same CT class. The `y1-trampoline-state[0..7]` field still holds the real per-track UID internally so T4 / T5 detect edges and emit CHANGED proactively — only the on-wire payload is constant.
- `TrackInfoWriter.onTrackEdge()` now dedups the position-anchor reset by audio_id. Previously it unconditionally cleared `mPositionAtStateChange` to `0` and stamped `mStateChangeTime = elapsedRealtime()` on every `OnPreparedListener` callback. Some player engines fire `prepareAsync` (and the OnPrepared callback) on pause→resume cycles of the same track, not just on real track changes. New flow: snapshot the previous `mCachedAudioId`, call `flushLocked()` (which re-derives the current audio_id from `PlayerService.getPlayingSong()`), compare. If the audio_id actually changed, reset the position anchor and re-flush; otherwise the first flush has already refreshed metadata + duration without disturbing T6 / T9's live-extrapolation baseline.
- Pre-emit TRACK_CHANGED before `prepareAsync` completes. Patch B5.2b in `patch_y1_apk.py` hooks `PlayerService.toRestart()` right after each `setDataSource(newPath)` call (3 sites: audiobook IJK, music IJK, music AOSP MediaPlayer) to invoke a new `PlaybackStateBridge.onEarlyTrackChange()` helper. By the time `setDataSource` has been called the music app has already updated `mPlayingMusic` / `mPlayingAudiobook` to the new song, so `TrackInfoWriter.flushLocked()` writes the new track's metadata to `y1-track-info` immediately and `wakeTrackChanged()` fires the `metachanged` broadcast → T5 emits wire CHANGED. Previously the same chain ran from `OnPreparedListener` ~100-500 ms later (the `prepareAsync` decoder-warmup window). Same-track invocations (resume-from-pause, screen-on restart) hit `onTrackEdge`'s audio_id dedup → metadata-refresh-only path → T5 sees no edge in `state[0..7]` vs `file[0..7]` → no spurious wire CHANGED. The existing `OnPreparedListener` chain still runs at prepareAsync completion; both branches no-op at the wire level thanks to dedup.
- `TrackInfoWriter.onSeek()` now suppresses the music app's "resume from saved progress" seek that fires after every `prepareAsync` completion. `PlayerService.playerPrepared()` (lines 1737/1793/1923 in stock 3.0.2) calls `setCurrentPosition(savedTime)` to restore `restoreStartTime` / `Bookmark.startTime` / `Progress.startTime` — this is the music app's "resume where you left off" feature. Pre-fix, that seek went through our `onSeek` hook and overwrote the reset-to-0 from `onEarlyTrackChange`, leaving the AVRCP wire-side playhead showing the user's prior pause point on the freshly-skipped track instead of `0:00`. Post-fix, `onTrackEdge` stamps `mLastFreshTrackChangeAt = elapsedRealtime()` on real audio_id-changed transitions, and `onSeek` early-returns if invoked within 2 s of that stamp. User-initiated seeks (drag the seek bar) come well after 2 s so they're unaffected; the local Y1 UI's audio-side seek-to-resume still happens (we only suppress the AVRCP-side `mPositionAtStateChange` mirror).
