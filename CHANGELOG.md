...
- **Foreground activity propagates unhandled discrete media keys (Patches H + H′).** `BaseActivity.dispatchKeyEvent` historically returned TRUE for every KeyEvent regardless of whether the activity acted, swallowing `KEYCODE_MEDIA_PLAY` / `_PAUSE` / `_STOP` before they could reach `AudioService` → `PlayControllerReceiver`. Patch H returns `false` for those three keycodes so Patch E's discrete arms run for AVRCP-driven PASSTHROUGH 0x44/0x45/0x46. Patch H′ applies the same fix to `BasePlayerActivity.dispatchKeyEvent`, the music-player superclass that overrides `dispatchKeyEvent` itself and bypasses `BaseActivity` entirely — without H′, the discrete keycodes hit `BasePlayerActivity.onKeyUp`, fail every KeyMap check (KeyMap covers KEY_LEFT 88 / KEY_RIGHT 87 / KEY_MENU 4 / KEY_ENTER 66 / KEY_PLAY 85 only), and `return v1=1` silently consumes them. Both patches live entirely inside the music app's APK so `/system/usr/keylayout/AVRCP.kl` stays stock and other foreground apps (e.g. Rockbox) keep spec-correct keycode mapping.
