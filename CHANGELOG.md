...
### Changed
- `tools/install-android-sdk.sh`: tighten the JDK version check. Previously accepted any JDK ≥ 17; now warns when JDK > 21 because AGP 8.7.3 only supports 17–21 and newer JDKs fail at build time with `Toolchain ... does not provide the required capabilities: [JAVA_COMPILER]`. The warning includes the install incantation for JDK 17 on each major distro and the `JAVA_HOME` setup. Doesn't block sdkmanager (which works fine on newer JDKs); the constraint is only at the gradle build step.
- `docs/ANDROID-SDK.md`: rewrite the JDK requirement section. Now explicit about the 17–21 range, the `-devel` / `-jdk` package suffix gotcha (plain `java-17-openjdk` ships JRE only — no `javac` — same `[JAVA_COMPILER]` error), and how to point `JAVA_HOME` at JDK 17 even when a newer JDK is the system default.
- `src/Y1MediaBridge/README.md`: Build section now calls out the JDK 17–21 constraint inline and links at the relevant `docs/ANDROID-SDK.md` section.
