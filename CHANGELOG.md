...
- **Live play status with millisecond-precision position.** PLAYING / PAUSED / STOPPED edges land on the wire as soon as the Y1 transitions; position updates at 1 s cadence while playing, with track-edge anchoring keeping CT-side and Y1-side playheads in lockstep across pause/resume.
...
- **`Y1Bridge` Android service** (`src/Y1Bridge/`, package `com.koensayr.y1.bridge`) — minimal Binder host that satisfies MtkBt's `bindService(com.android.music.MediaPlaybackService)`. Built via Gradle. The companion in-music-app components (TrackInfoWriter / PlaybackStateBridge / BatteryReceiver / PappStateBroadcaster / PappSetFileObserver) handle every observation, state production, and broadcast path.
...
- **In-app `AvrcpBinder` smali** (`com.koensayr.y1.avrcp.*` — Patch B6) lands in `classes2.dex` as groundwork for a future architecture where MtkBt binds directly into the music-app process. Currently unused at runtime — `Y1Bridge.apk` hosts the actual Binder MtkBt resolves to, because the music APK's manifest can't be modified safely: `com.innioasis.y1` declares `sharedUserId="android.uid.system"` which constrains signing to the OEM platform key, and any AndroidManifest.xml change invalidates `META-INF/MANIFEST.MF`'s SHA1-Digest.
...
- **Bridge APK shrunk to a minimal Binder host.** ~91% source reduction. The bridge no longer scrapes logcat, writes files, registers `RemoteControlClient`, listens for battery / PApp broadcasts, or fires media broadcasts — all of that lives in the music app's process now. The bridge's only purpose is presenting a `<service>` declaration with the `com.android.music.MediaPlaybackService` intent-filter (which the music app can't declare itself — sharedUserId / platform-key constraint). `onBind` returns an `AvrcpBinder` whose `onTransact` ack-only's every code except `getCapabilities` (returns `[0x01, 0x02]`). MtkBt never transacts past `getCapabilities` in practice (Sonos capture verified); the broadcast wake path drives every CT-visible AVRCP event.
...
- `src/Y1MediaBridge/` → `src/Y1Bridge/`; Java package `com.y1.mediabridge` → `com.koensayr.y1.bridge`; `Y1MediaBridge.apk` → `Y1Bridge.apk`; `PlaySongReceiver` → `BootReceiver` (reflecting its sole remaining responsibility); log tag `Y1MediaBridge` → `Y1Bridge`. Project artifacts now consistently in the Koensayr namespace. Historical CHANGELOG entries below this release retain the old names since they describe artifacts that shipped at those versions.
