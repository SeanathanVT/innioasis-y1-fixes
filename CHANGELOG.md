...
### Fixed
- `tools/install-android-sdk.sh`: license-accept step was failing silently — `>/dev/null` on the line `yes | sdkmanager --licenses >/dev/null` swallowed any error message; `set -euo pipefail` then aborted the script with no visible output, before the component install or `local.properties` write could run. Removed the redirect, wrapped both `sdkmanager` invocations (`--licenses` and `--install`) in explicit `if !` blocks that print a useful error (including current `java -version`) and the manual-debug invocation when they fail.
