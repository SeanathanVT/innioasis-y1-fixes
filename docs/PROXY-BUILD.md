...
- [x] **iter10: advertise only TRACK_CHANGED** — empirically Sonos aborts the registration loop on the first NOT_IMPLEMENTED (iter9 saw only 1 size:13, then nothing). Pre-iter9 the unknow path was broken so Sonos timed out and accidentally tried event 0x02 anyway. With proper rejects flowing, advertising only event 0x02 (TRACK_CHANGED) keeps Sonos walking the full handshake. Two-byte change: T1's events count `5 → 1` and events_data `01 02 09 0a 0b → 02`. **Pending hardware verification.** Output md5: `b5baaa7ad25917612465022df46ec1ea`.

Pending (this document is the plan):
- [ ] Hardware test iter10 — verify Sonos sends size:13 for event 0x02 (TRACK_CHANGED), T2 acks with msg=544, and Sonos progresses to size:45 (GetElementAttributes).
