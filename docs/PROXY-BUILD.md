...
- [x] **iter12: multi-attribute T4 dispatch (loop, separate frames)** — extends T4 to 152 bytes, parses inbound num_attributes + attribute_ids, dispatches on attribute_id LSB to per-attribute hardcoded strings. Each requested+supported attribute generated one msg=540 outbound. **Hardware-verified iter12**: 3:1 ratio of msg=540:size:45 confirmed multi-call works; Sonos displayed "Y1 Title" only — used the first frame and treated subsequent frames with same transId as duplicates. Output md5: `fa6191d6ce8170f5ef5c8142202c8ba5`.
- [x] **iter13: multi-attribute single-frame response (correct semantics)** — after disassembling `btmtk_avrcp_send_get_element_attributes_rsp` at libextavrcp.so:0x2188, decoded its real argument layout: `arg2` = attribute INDEX (0..N-1), `arg3` = TOTAL count, function emits IPC frame only when `arg2+1 == arg3 AND arg3 != 0` — earlier calls accumulate into a 644-byte internal buffer. iter13 makes 3 sequential calls with `arg2=0/1/2`, `arg3=3` → calls 1+2 accumulate, call 3 emits ONE frame containing Title+Artist+Album. T4 size 148 bytes. LOAD #1 grows to 0xace8. transId comes from `conn[17]` inside the function (not an arg). **Pending hardware verification.** Output md5: `56d9d8514f30a12aaf2303b7a7f6a067`.

Pending (this document is the plan):
- [ ] Hardware test iter13 — Sonos should display title=Y1 Title, artist=Y1 Artist, album=Y1 Album simultaneously. Ratio of msg=540:size:45 should drop to 1:1.
- [ ] iter14: Y1MediaBridge writes `/data/local/tmp/y1-track-info` on track change; T4 reads it via open/read/close instead of using hardcoded strings. Trampoline grows ~80 bytes for the file I/O. (~4128 bytes available in LOAD #1 padding.)
