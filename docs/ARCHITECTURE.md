...
How the Innioasis Y1 delivers AVRCP 1.3 metadata (Title/Artist/Album/TrackNumber/TotalNumberOfTracks/Genre/PlayingTime) to peer Controllers, given that the OEM Bluetooth stack is fundamentally an AVRCP 1.0 implementation that auto-rejects 1.3+ commands. We advertise 1.3 over AVCTP 1.2 (see `patch_mtkbt.py` V1/V2, with ESR07 §2.1 / Erratum 4969 SDP-record clarifications applied) and implement the 1.3 metadata feature set: `GetCapabilities` 0x10, `InformDisplayableCharacterSet` 0x17, `InformBatteryStatusOfCT` 0x18, `GetElementAttributes` 0x20 (all 7 §5.3.4 attributes packed in a single response), `GetPlayStatus` 0x30 (with `clock_gettime(CLOCK_BOOTTIME)` live-position extrapolation), and `RegisterNotification` 0x31 with INTERIM coverage of events 0x01..0x08 + 0x09..0x0c and proactive CHANGED-on-edge for events 0x01 / 0x02 / 0x05 / 0x06 / 0x08. The advertised set (0x01, 0x02, 0x05, 0x08, 0x09, 0x0a, 0x0b, 0x0c) mirrors Pixel-as-TG; the four 1.4+ event IDs 0x09-0x0c are INTERIM-only with zero payload (Y1 has one player, no Now Playing folder, no UID database) and exist on the advertised list because strict CT metadata-pane render empirically gates on their being acked even when the SDP profile descriptor advertises 1.3. F1's MtkBt-internal-version flip is a Java-side dispatcher-unblock flag (BlueAngel internal value), not a wire-shape upgrade. See [`BT-COMPLIANCE.md`](BT-COMPLIANCE.md) §0 for spec citation discipline and §2 for the ICS Table 7 coverage scorecard.
...
| M1a / M1b | mtkbt 0x1222e / 0x12230 | RegNotif response discriminator retarget: load ctxt[7] (where JNI helper writes reasonCode 0x0F/0x0D) instead of ctxt[8] (always zero from helper memset); cmp against 0x0F. Wire ctype now matches whatever reasonCode the trampoline passes — INTERIM (0x0F) for T2 / T8 arms, CHANGED (0x0D) for T5 / T9 edge emits. |
...
The wire-level `Identifier` field in TRACK_CHANGED notifications is pinned to `0x0000000000000000` SELECTED — the AVRCP 1.4+ value meaning "the currently playing media is selected" (versus `0xFFFFFFFFFFFFFFFF` which means "no media currently selected" per AVRCP 1.3 §5.4.2 Tbl 5.30 + ESR07 §2.2 clarifying the 8-byte form). Pixel-as-TG sends SELECTED to Bolt and Bolt renders the metadata pane; the 1.3 no-track sentinel reads as "nothing playing" to strict 1.4+ CTs and suppresses the pane. The `y1-trampoline-state[0..7]` field still holds the real per-track UID internally so T4 / T5 can detect edges and emit CHANGED proactively — only the on-wire 8-byte payload is constant. An older "real synthetic id on the wire" variant caused a ~90 Hz `RegisterNotification` subscribe storm against high-subscribe-rate CTs that saturated AVCTP and dropped PASSTHROUGH release frames; SELECTED avoids that storm because the payload doesn't change between subsequent TRACK_CHANGED CHANGEDs.
