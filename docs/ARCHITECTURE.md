...
| extended_T2 + T4 + T5 + T_charset + T_battery + T6 | jni 0xac54 (892 B, iter20a) | NEW LOAD #1 extension, dynamically assembled. **extended_T2** (reactive RegisterNotification): reads track_id from y1-track-info into a stack buffer, writes [track_id\|\|transId\|\|pad] to y1-trampoline-state, replies INTERIM with `arg1=0` + REASON_INTERIM + **&sentinel_ffx8** (iter16 sentinel). **T4** (reactive GetElementAttributes): reads both files; if state[0..7] != file[0..7], emits track_changed_rsp CHANGED with `arg1=0` + reason=CHANGED + **&sentinel_ffx8** + writes new state, then replies *one* multi-attribute get_element_attributes_rsp frame containing Title / Artist / Album. **T5** (proactive on Y1 track change, iter17a): entered via b.w from the patched `notificationTrackChangedNative`; emits CHANGED with `arg1=0` + reason=CHANGED + **&sentinel_ffx8** on track_id divergence, updates state. Returns jboolean(1). **T_charset / T_battery** (iter19a — Phase A0): minimal trampolines for PDUs 0x17 / 0x18 (CT→TG informational pair) — call `inform_charsetset_rsp` / `battery_status_rsp` with `arg1=0` and tail-jump to the function epilogue. **T6** (iter20a — Phase B): PDU 0x30 GetPlayStatus — reads y1-track-info[776..795] (duration_ms / pos_at_state_change_ms / state_change_time_sec / playing_flag, all big-endian on disk), byte-swaps via Thumb-2 REV instruction (new `rev_lo_lo` in `_thumb2asm.py`), and calls `get_playstatus_rsp` via PLT 0x3564 with the host-order values. T4's pre-check dispatches PDU 0x20 → main body, 0x17 → T_charset, 0x18 → T_battery, **0x30 → T6**, else fall through to "unknow indication" (0x65bc). |
...
| LOAD #1 padding | 0xac54..0xbc07 | 4276 bytes | T4 (~328 B) + extended_T2 (~140 B) + T5 (~180 B) + T_charset (14 B) + T_battery (14 B) + T6 (~52 B) + path strings (~108 B) + sentinel (8 B) = 892 B used (iter20a), ~3384 free |
