...
The wire-level `Identifier` field in TRACK_CHANGED notifications carries the per-track audio_id (BE u64, read from `y1-track-info[0..7]`). Strict 1.4+ CTs (Bolt-class) cache `GetElementAttributes` responses keyed by the TRACK_CHANGED Identifier; an unchanging value (e.g. the `0x0000000000000000` SELECTED sentinel) leads to Bolt only refreshing the metadata pane on the FIRST CHANGED of a session and then dedup'ing every subsequent one — observed empirically as Bolt updating ~50% of track changes when SELECTED was used. Per-track audio_id forces cache invalidation + re-query on every track edge. `y1-trampoline-state[0..7]` holds the previous audio_id so T4 / T5 can edge-detect on real-id transitions before emitting CHANGED. An older "real synthetic id" variant against Sonos hit a ~90 Hz `RegisterNotification` subscribe storm — but that was Sonos-specific (it re-registered before each CHANGED in lockstep with our 70Hz reactive emit cadence); under the proactive Pixel-mirror emit pattern (T5 / T9 fire on file-edge, not on poll) the per-track-id approach produces a steady ~1 CHANGED per actual track edge, well below storm thresholds.
