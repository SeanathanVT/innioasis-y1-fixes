...
The wire-level `Identifier` field in TRACK_CHANGED notifications is pinned to `0x0000000000000000` SELECTED — the AVRCP 1.4+ value meaning "the currently playing media is selected" (versus `0xFFFFFFFFFFFFFFFF` which means "no media currently selected" per AVRCP 1.3 §5.4.2 Tbl 5.30 + ESR07 §2.2 clarifying the 8-byte form). Strict 1.4+ CTs read the 1.3 no-track sentinel as "nothing playing" and suppress the metadata pane; SELECTED keeps the pane rendering. `y1-trampoline-state[0..7]` still holds the real per-track UID internally so T4 / T5 detect edges and emit CHANGED proactively — only the on-wire 8-byte payload is constant. SELECTED also avoids a subscribe-storm against high-subscribe-rate CTs because the payload doesn't change between successive CHANGEDs.
