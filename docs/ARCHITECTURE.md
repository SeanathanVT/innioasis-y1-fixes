...
| extended_T2 + T4 + T5 + T_charset + T_battery | jni 0xac54 (792 B, iter19b) | NEW LOAD #1 extension, dynamically assembled. **extended_T2** (reactive RegisterNotification): reads track_id from y1-track-info into a stack buffer, writes [track_id\|\|transId\|\|pad] to y1-trampoline-state, replies INTERIM with the **real track_id** (iter19b — was 0xFF×8 sentinel in iter16-iter19a). **T4** (reactive GetElementAttributes): reads both files; if state[0..7] != file[0..7], emits track_changed_rsp CHANGED with `arg1=0` + reason=CHANGED + **&file[0..7]** (iter19b) + writes new state, then replies *one* multi-attribute get_element_attributes_rsp frame containing Title (file+8) + Artist (file+264) + Album (file+520) — see [Reverse-engineered semantics: `btmtk_avrcp_send_get_element_attributes_rsp`](#reverse-engineered-semantics-btmtk_avrcp_send_get_element_attributes_rsp). **T5** (proactive on Y1 track change, iter17a): entered via b.w from the patched `notificationTrackChangedNative`; calls the JNI helper at 0x36c0 to obtain the per-conn struct, reads both files, emits CHANGED with `arg1=0` + reason=CHANGED + **&file_tid_buf** (iter19b — real track_id) on track_id divergence, updates state. Returns jboolean(1). **T_charset / T_battery** (iter19a — Phase A0): minimal trampolines for PDUs 0x17 / 0x18 (CT→TG informational pair) — call `inform_charsetset_rsp` / `battery_status_rsp` with `arg1=0` and tail-jump to the function epilogue. T4's pre-check dispatches PDU 0x20 → main body, 0x17 → T_charset, 0x18 → T_battery, else fall through to "unknow indication" (0x65bc). |
...
- **iter19b** drops the sentinel; restores iter15's "real `track_id` on the wire" behavior. Safe now because T5 supplies the proactive CHANGED edges that iter15 lacked. Strict CTs (Chevrolet Bolt EV) that gate metadata refresh on the `track_id` actually changing — and ignored every iter16/17/19a CHANGED because they all carried the same `0xFF×8` — now see real changes per CHANGED edge.

The `track_id` we send is the same 8-byte value Y1MediaBridge writes to `y1-track-info[0..7]`: the synthetic audioId from `path.hashCode() | 0x100000000L` (iter18d) — unique per track, stable across re-plays of the same path.
...
| LOAD #1 padding | 0xac54..0xbc07 | 4276 bytes | T4 (~324 B) + extended_T2 (~140 B) + T5 (~180 B) + T_charset (14 B) + T_battery (14 B) + path strings (~108 B) + sentinel (8 B, kept harmlessly post-iter19b) = 792 B used, ~3484 free |
