...
- **Patch E** (iter22d) in `smali_classes2/com/innioasis/y1/receiver/PlayControllerReceiver.smali` at `:cond_c` — extends the `KEY_PLAY → playOrPause()` branch to also fire on `KEYCODE_MEDIA_PLAY` (126) and `KEYCODE_MEDIA_PAUSE` (127). Stock has a single `if-ne v2, p1, :cond_e` after `getKEY_PLAY()`; iter22d replaces it with `if-eq v2, KEY_PLAY, :cond_play_match` + `const/16 p1, 0x7e` + `if-eq v2, p1, :cond_play_match` + `const/16 p1, 0x7f` + `if-ne v2, p1, :cond_e`. Both new keyCodes share the same toggle action via a new `:cond_play_match` label. Fixes Kia EV6 head-unit discrete PLAY button (which sends Linux `KEY_PLAY` = 207 → Android `KEYCODE_MEDIA_PLAY` = 126, not the `KEY_PLAYPAUSE` = 201 → `KEYCODE_MEDIA_PLAY_PAUSE` = 85 that the stock `KeyMap.KEY_PLAY` is hardwired to).
