...
On match, reads both live values via `SharedPreferencesUtils.INSTANCE.getMusicRepeatMode()` / `getMusicIsShuffle()`, maps to the AVRCP enum bytes, calls `TrackInfoWriter.setPapp(repeat, shuffle)` so the music-app `y1-track-info[795..796]` reflects the new state immediately, and broadcasts `com.y1.mediabridge.PAPP_STATE_DID_CHANGE` so a `playstatechanged` wake propagates to MtkBt → T9 → AVRCP §5.4.2 Tbl 5.36 `PLAYER_APPLICATION_SETTING_CHANGED` CHANGED via PLT `0x345c`.
...
| `smali/com/koensayr/PappStateBroadcaster.smali` (B4 product) | `sendNow()` tail — also calls `TrackInfoWriter.setPapp(repeat, shuffle)` so the music-app file reflects the new state immediately. |
