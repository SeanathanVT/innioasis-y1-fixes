...
| Inject-tree entry traces | `TrackInfoWriter` — `init / setPlayStatus / onSeek / markCompletion / markError / onTrackEdge / setBattery / setPapp / flush / flushLocked / wakeTrackChanged / wakePlayStateChanged`. `PlaybackStateBridge` — `onPlayValue / onEarlyTrackChange / onPrepared / onCompletion / onSeek / onError`. `PositionTicker` — `start / stop / run`. `BatteryReceiver` — `register / onReceive`. `PappSetFileObserver` — `start / onEvent / dispatch`. `NowPlayingRefresher` — `onResume / onPause / refresh / run`. |
