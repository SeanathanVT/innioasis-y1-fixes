...
- **Patch D** (iter21) — three sub-edits bounding the music app's hold-to-fast-forward / hold-to-rewind cascade so a dropped PASSTHROUGH-release frame (observed under AVCTP saturation when a CT subscribes at high frequency — see [`INVESTIGATION.md`](INVESTIGATION.md) for the per-CT context) recovers within ~5 s instead of running forever:
