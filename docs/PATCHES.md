...
`fcn.0xf0bc` dispatches outbound AVRCP responses through two structurally-identical builders selected by `packet[9]`, which `fcn.0x11894` sets from `(msg_id > 6)` via an `ite hi` block at file offset `0x1190a`: msg_id > 6 (all common AVRCP response IDs — `msg=540` GEA, `msg=544` RegNotif INTERIM/CHANGED, `msg=520`, `msg=522` GetCapabilities, etc.) writes `packet[9] = 0` and takes Path B via `fcn.0xef08 → fcn.0x6d0f0`; msg_id ≤ 6 (rare AVCTP-control messages) writes `packet[9] = 1` and takes Path A via `fcn.0xed50 → fcn.0x6d048`. M2/M3 patch Path A; M4 patches Path B.

`fcn.0x6d0f0` is byte-for-byte structurally identical to M2's `fcn.0x6d048`: same `fcn.0x6ccdc` list-contains check at `0x6d110`, same INTERIM/CHANGED discriminator at `0x6d11e`, same drop target `movs r0, 0xd; pop {r3, r4, r5, pc}` at `0x6d19c`. The list-contains check empirically gates the majority of Path B emits on subscription-class CTs under sustained traffic. M4 NOPs the analogous `beq 0x6d19c` at `0x6d116`, so `fcn.0x6d0f0` unconditionally builds the wire frame and tail-calls `b.w 0xae5e4` (`L2CAP_SendData`). `fcn.0x6d0f0` skips `fcn.0x6df20` entirely, so M3's chip-busy SET has no analogue on Path B.

**M6 — Path B TID slot: lift transId from IPC[5] into packet[0xd]** at file `0x11924` (1 site, 2 bytes):

| site | bytes (before → after) | mnemonic |
|---|---|---|
| `0x11924` | `00 26` → `4e 79` | `movs r6, 0` → `ldrb r6, [r1, 5]` |

Path B (`fcn.0x6d0f0`) at file offset `0x6d186` reads `packet[0xd]` and at `0x6d188` stores it into `chan[0x29]` — the §6.7.2 notification-TID slot consumed by `fcn.0xae418`'s wire-byte-0 builder (selected when `buffer[+0x08] != 0`, which is Path B's stock value of 2; Path A uses the `chan[0x28]` AV/C command-response TID slot per §6.5 instead, selected by `buffer[+0x08] == 0`). Stock `fcn.0x11894` initialises `packet[0xd] = 0` unconditionally at `0x11924-0x11927` (`movs r6, 0; strb r6, [r4, 0xd]`), so every Path B wire frame emits with TID = 0 regardless of the originating AV/C TID. CTs whose RegisterNotification CMD uses TID = 0 match by coincidence (`get_element_attributes_rsp` deliveries land); CTs that use non-zero RegNotif TIDs see the response fail the §6.7.2 TID-echo check, drop silently at the AVRCP layer, and re-fire the AVCTP V13 §3.3.5 3-s response-timeout retry timer.

`libextavrcp.so`'s response builders (`btmtk_avrcp_send_reg_notievent_*_rsp`, `..._get_element_attributes_rsp`, `..._get_capabilities_rsp`, etc.) all write `ipc[5] = conn[0x11]` — the latched inbound transId. M6 replaces the constant-zero source with `ldrb r6, [r1, 5]`, where r1 holds the IPC payload pointer (loaded at `0x11910` `ldr r1, [arg_28h]` and not modified through `0x11923`). The unchanged `strb r6, [r4, 0xd]` at `0x11926` then propagates the real transId into `packet[0xd]`. Path B's downstream chain emits wire byte 0 = `(transId << 4) + 4 + 2`, matching the originating RegisterNotification CMD's transId per AVRCP 1.3 §6.7.2.

r6 is consumed by the prior `strb r6, [r4, 0xb]` at `0x11922` (msg_id low byte → packet[0xb]) and unused after the strb at `0x11926` within the function, so repurposing it is local. Path A also flows through `fcn.0x11894` and reads `packet[0xd]` via `fcn.0xed50` at `0xedee` → chan[0x31b], but its wire-frame builder doesn't consume chan[0x31b] — the changed value is dead state on Path A.

**MD5s:** Stock `3af1d4ad8f955038186696950430ffda` → Output `3c814fb2715d7919c38b04126e1ec3e2`.
