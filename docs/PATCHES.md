...
In the LOAD #1 padding region, reached from extended_T2's PDU-0x20 dispatch arm. Eight sequential calls to `btmtk_avrcp_send_get_element_attributes_rsp` via PLT `0x3570` with `arg2=0..7`, `arg3=8` — only the eighth call (where `arg2+1 == arg3`) emits a frame, packing all eight attributes (AVRCP 1.3 §5.3.4 attrs 0x01-0x07 plus AVRCP 1.6 §5.13.4 attr 0x08) into a single response:
...
The music app is the canonical writer of the 1112-byte `y1-track-info` schema, the 16-byte `y1-trampoline-state` (initial create), and the 2-byte `y1-papp-set` (initial create). All three live in `/data/data/com.innioasis.y1/files/`. The trampoline chain in `libextavrcp_jni.so` reads them directly.
