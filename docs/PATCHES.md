...
| Trampoline-state byte dump | `TrackInfoWriter.wakeTrackChanged` → `wTC.pre tramp.state[13..19]=…`. `TrackInfoWriter.wakePlayStateChanged` → `wPSC.pre tramp.state[13..19]=…`. Reads `y1-trampoline-state[13..19]` (the AVRCP 1.3 §6.7.1 per-subscription gates: state[13]=TRACK_CHANGED, state[14]=PLAYBACK_STATUS, state[15]=POS, state[16]=BATT, state[17]=PAPP) immediately before each `sendBroadcast`. Successive captures answer whether T5/T9 actually emitted CHANGED on the wire (gate cleared between wakes) or got gated out (gate stayed armed because the prior wake didn't fire either, or because the CT hasn't re-RegisterNotification'd yet per §6.7.1). |
