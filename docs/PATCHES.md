...
**M5 — Path B AVCTP wire-shape align: buffer[0x08] from 2 to 0** at file `0x6d13b` (1 site, 1 byte):

| site | bytes (before → after) | mnemonic |
|---|---|---|
| `0x6d13b` | `10` → `20` | `strb.w r1, [r4, 0xe0]` → `strb.w r2, [r4, 0xe0]` |

Path A (`fcn.0x6d048`) writes 0 to its frame buffer at relative offset `+0x08` (`strb.w r3, [r4, 0xc8]` with r3=0 at `0x6d086`). Path B (`fcn.0x6d0f0`) wrote 2 (`strb.w r1, [r4, 0xe0]` with r1=2 at `0x6d138`). `fcn.0xae418` consumes this byte twice in the single-AVCTP-packet branch (the only branch our wire sizes ever hit — all observed responses fit < 502 B). At `0xae43c` it is a discriminator that selects which chan-struct TID slot (`chan[0x14]` if zero, `chan[0x15]` otherwise) becomes bits 7:4 of AVCTP wire byte 0; at `0xae4f2` the same byte is reread and OR'd into bits 3:0 (AVCTP V13 §3.3 Cr / IPID / PktType). Path A's pattern (`chan[0x14]` TID, CR=0) is empirically what subscription-class CTs accept; Path B's pattern (`chan[0x15]` TID, CR=1) is silently dropped wire-side.

M5 re-encodes the Thumb-2 strb.w T2 imm12 at `0x6d138` to write from r2 (which `movs r2, 0` at `0x6d132` set to 0) instead of r1 (set to 2 at `0x6d12e`). Halfword 2 of the encoding carries `Rt:4 imm12:12`; Rt=1 → bytes `e0 10`, Rt=2 → bytes `e0 20`. r1 stays dead until `0x6d150` (`and r1, r0, 0xf`), r2 stays dead until `0x6d180` (`ldr r2, [r5, 8]`), so the change is surgically local. Both wire-byte-0 dimensions flip together — see `docs/INVESTIGATION.md` Trace #51 for the byte-level disassembly diff that localized the divergence.

**MD5s:** Stock `3af1d4ad8f955038186696950430ffda` → Output `52a4ab9f50d4f5293421324d1a5dcd84`.
