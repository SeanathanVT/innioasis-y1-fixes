...
**Patch B6** — AvrcpBridgeService + AndroidManifest splice (the Binder MtkBt binds to).

Two new classes routed to `smali_classes2/` (secondary DEX) because `classes.dex` sits at 99.7% of the 64K method cap after Patch B5. apply.bash invalidates the MultiDex code_cache before pushing the APK so Dalvik picks up the new `classes2.dex`.

| Class | Role |
|---|---|
| `avrcp.AvrcpBridgeService` | Exported `Service`. `onBind` returns an `AvrcpBinder`; `onCreate` stashes `sInstance` so callers can read `getApplicationContext()` for media-key broadcasts. Static `notifyPlaybackStatus(B)` / `notifyTrackChanged(J)` entry points iterate the registered IBTAvrcpMusicCallback IBinders and `transact()` them; failed transacts are removed from the list (RemoteException safety). |
| `avrcp.AvrcpBinder` | `Binder` implementing the `IBTAvrcpMusic` + `IMediaPlaybackService` onTransact protocol. Skips `strictModePolicy` + descriptor string and dispatches purely by transact code — `enforceInterface` is brittle because the descriptor strings MtkBt writes can drift across ROM variations and silently abort `registerCallback` (code 1) on mismatch. Codes implemented: 1 (`registerCallback`) — stash IBinder via `AvrcpBridgeService.addCallback`; 2 (`unregisterCallback`); 3 (`regNotificationEvent`) — ACK true; 4 (`setPlayerApplicationSettingValue`) — ACK true; 5 (`getCapabilities`) — return `[0x01 PLAYBACK_STATUS_CHANGED, 0x02 TRACK_CHANGED]`; 6-13 (`play`/`stop`/`pause`/`resume`/`prev`/`next`/`prevGroup`/`nextGroup`) — broadcast DOWN+UP `ACTION_MEDIA_BUTTON` keyevents to `PlayControllerReceiver` via `sendMediaKey`. Every other code: `writeNoException` + `return true` (ack-only). The C-side trampolines deliver the real metadata + control on the AVRCP wire, so ack-only Java responses are sufficient. |

`AndroidManifest.xml` splice via `_axml.py`: inserts a `<service android:name="com.koensayr.y1.avrcp.AvrcpBridgeService" android:exported="true">` with `<intent-filter android:priority="100">` containing `<action android:name="com.android.music.MediaPlaybackService"/>` and `<action android:name="com.android.music.IMediaPlaybackService"/>` just before `</application>`. `_axml.py` is a minimal Python binary-AXML editor that round-trips the existing manifest (verified byte-identical on no-op), appends new strings to the pool beyond the resource-mapped prefix (existing indices stay valid), and splices new Start/End element chunks at a chosen position.

**Apktool reassembly:** `apktool d --no-res` decode → smali edits → manifest splice via `_axml.py` → `apktool b` reassemble (the post-DEX aapt step fails because resources weren't decoded, but DEX is already built by then; the script intentionally ignores the exit code). Patched DEX bytes + patched `AndroidManifest.xml` are dropped into a copy of the original APK with `META-INF/` preserved.
