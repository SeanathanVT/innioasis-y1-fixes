...
- **2026-05-08 postflash (`/work/logs/dual-sonos-postflash/`):** resume-from-pause needed double-tap. AVRCP 0x44 PLAY arrived at kernel as `KEY_PLAYCD` (7 events confirmed in `getevent.txt`) but `Y1Patch: PlayerService.play(Z) entry` never fired in the music app. Root cause: `MediaBridgeService.setupRemoteControlClient` claimed the system MediaButton-receiver slot via `AudioManager.registerMediaButtonEventReceiver(PlaySongReceiver)`; `PlaySongReceiver` was supposed to forward `ACTION_MEDIA_BUTTON` to `PlayControllerReceiver`, but its `MEDIA_BUTTON keyCode=` forwarding log fired zero times — neither us nor the music app held the slot at AVRCP-press time. Closed 2026-05-08 (Y1MediaBridge versionCode 21→22) by removing `registerMediaButtonEventReceiver` and the matching manifest filter; AudioService's ordered-broadcast fallback now delivers AVRCP keys directly to `PlayControllerReceiver` (priority MAX_VALUE).
...
- **2026-05-08 postflash (`/work/logs/dual-tv-postflash/`):** stuck `KEY_PAUSECD DOWN` in `getevent.txt` for ~15 s before the matching UP arrived (epoch `1778236775` → `1778236830`). High RegisterNotification subscribe-storm cardinality (`size:13` to `size:45` ratio ≈ 3.7:1) suggests the TV is re-subscribing because expected CHANGED edges aren't arriving fast enough. Most likely an artifact of the slot-stealing bug above (TG missing PASSTHROUGH releases under load) plus possibly a separate TV-side sequencing quirk. Re-test post-2026-05-08 fix to separate the two.
...
- **2026-05-08 postflash (`/work/logs/dual-bolt-postflash/`):** two distinct findings. (1) **Pause-during-play broken from the user's perspective, but the Bolt sends NO PAUSE op_id at all.** `getevent.txt` over the entire session shows only `KEY_PLAYCD`, `KEY_NEXTSONG`, `KEY_PREVIOUSSONG` arriving at `/dev/input/event4` — never `KEY_PAUSECD`. The Bolt's UI "pause" button transmits nothing on the AVRCP wire; its on-screen icon-flip is local UI optimism diverging from the Y1's actual state. Not fixable from TG side. (2) **PLAY-resume same root cause as Sonos** (slot-stealing); closed by the same 2026-05-08 fix. (3) **No metadata** displayed by the Bolt despite `msg=540` GetElementAttributes being emitted with all 7 §5.3.4 attrs — separate Bolt-side ingestion issue, open investigation.
...
- **2026-05-08 postflash (`/work/logs/dual-kia-postflash/`):** play-during-pause broken — same root cause as Sonos (slot-stealing). 0x44 PLAY → `KEY_PLAYCD` reached kernel cleanly, no `play(Z) entry` fired. Closed by the 2026-05-08 MediaButton-receiver-deregistration fix. Track playing time + scrub bar advance verified working on this CT.
