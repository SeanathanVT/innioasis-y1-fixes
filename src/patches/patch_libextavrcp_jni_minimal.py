...
Output md5:        86a3d52c03baa94898491a6e9764d69a
...
OUTPUT_MD5 = "86a3d52c03baa94898491a6e9764d69a"
...
# 236 bytes. See ARCHITECTURE.md for instruction-level annotation. The structure is:
#   pre-check (20) + push (2) + sub.w sp,#784 (4) + memset (12) + open (8) +
#   fd check (4) + open-fail cleanup (16) + read syscall (12) + close (6) +
#   3 × per-attribute call (~38) + cleanup+epilogue (10) + filename (32)
T4_STUB = bytes([
    ...
    0x61, 0x2F, 0x6C, 0x6F, 0x63, 0x61, 0x6C, 0x2F, 0x74, 0x6D, 0x70, 0x2F, 0x79, 0x31, 0x2D, 0x74,
    0x72, 0x61, 0x63, 0x6B, 0x2D, 0x69, 0x6E, 0x66, 0x6F, 0x00, 0x00, 0x00,
])
assert len(T4_STUB) == 236

# Stock bytes at 0xac54..0xad40 — all zero (LOAD #1 page padding).
T4_STUB_STOCK = bytes([0x00] * 236)
