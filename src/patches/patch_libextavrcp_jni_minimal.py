...
Output md5:        b39ba1f4aca8b80b85f5001a7beff793
...
OUTPUT_MD5 = "b39ba1f4aca8b80b85f5001a7beff793"
...
# 256 bytes. iter14b reads metadata from a file Y1MediaBridge writes:
#   /data/data/com.y1.mediabridge/files/y1-track-info  (uid 10000 writable;
#   parent dirs world-x for traversal after MediaBridgeService chmods filesDir
#   via setExecutable(true,false); file is world-readable via setReadable).
#
# Structure:
#   pre-check (20) + push (2) + sub.w sp,#784 (4) + memset (12) + open (8) +
#   fd check (4) + open-fail cleanup (16) + read syscall (12) + close (6) +
#   3 × per-attribute call (~38) + cleanup+epilogue (10) + filename (52)
#
# If the file is missing/unreadable, T4 falls through gracefully via the
# open-fail path → b.w 0x65bc → msg=520 NOT_IMPLEMENTED. Never crashes.
T4_STUB = bytes([
    ...
    ,
])
assert len(T4_STUB) == 256

# Stock bytes at 0xac54..0xad54 — all zero (LOAD #1 page padding).
T4_STUB_STOCK = bytes([0x00] * 256)
