...
#
# §6.7.1 strict gate semantics: T2 / T8 INTERIM arms a gate byte = 1; T5 / T9
# CHANGED reads it, emits the response, then clears it back to 0. CT must
# RegisterNotification again to receive the next CHANGED for that event.
# This matches Pixel-as-TG observed cadence on Bolt: each CHANGED is
# followed within ~6-22 ms by a CT RegisterNotification + TG INTERIM, then
# ~1 s wait for the next CHANGED. Wire-side cadence becomes
# min(TG_tick_rate, CT_re-register_rate). Compliant CTs (Sonos / Bolt /
# Kia) re-register promptly so the loop runs at TG's PositionTicker rate.
#
...
# Per-event subscription gates (AVRCP 1.3 §6.7.1 strict). T2 / T8 INTERIM
# emit for a given event sets the matching byte = 1; T5 / T9 CHANGED emit
# reads, fires the response, then clears the byte = 0. CT must
# RegisterNotification again to re-arm. This matches Pixel-as-TG observed
# behaviour on Bolt (every CHANGED followed by ~6-22 ms CT re-register +
# fresh INTERIM, then 1 s wait for the next CHANGED). Earlier builds
# implemented "session-long armed" / no-clear semantics under the
# "Pixel-mirror" label; that label was based on a misreading of Pixel
# captures and was corrected 2026-05-15 once a clean btsnoop showed
# Pixel actually emits §6.7.1-strict.
# y1-trampoline-state on disk grows from 20 → 21 bytes on first
# sub_now_playing_content arm (T8 0x09 INTERIM via lseek+write past EOF
# zero-extends). Older 16-byte and 20-byte files degrade gracefully (read
# returns zero-fill on the new bytes; the gate evaluates as "not
...
def _emit_t5(a: Asm) -> None:
    ...
    # (state[20], armed by T8 INTERIM for 0x09); cleared after emit per
    # §6.7.1 strict — CT re-registers to receive the next track edge.
    ...
    # Reads file[780..783] BE → host. Gate on sub_pos (state[13]);
    # cleared after emit per §6.7.1 strict.
    ...
    # No clear here: T9's POS_CHANGED emit (PositionTicker path) clears
    # state[13] each tick during playback. T5 fires POS_CHANGED on track
    # edges in addition to T9's tick — keeping the gate armed across the
    # T5 track-edge burst lets T9 still see armed gate on the next 1 s
    # tick. CT re-register loop is owned by T9. (Blob-budget driven —
    # would otherwise add a redundant clear.)
    ...
    # AVRCP §6.7.1 strict: clear sub_track_changed (state[16]).
    ...
    # sub_track_reached_start bit at state[18] (cleared after emit per §6.7.1).
...
def _emit_subscription_write(a: Asm, byte_value: int, state_byte_offset: int,
    ...
    for the AVRCP §6.7.1 once-per-registration semantic. `fd_reg` (default 4)
    is the callee-saved register cached as fd between open and close —
    callee-saved so the value survives the intervening PLT blx calls per
    AAPCS r4-r11 preservation.

    Default fd_reg=4 keeps T2 / T8 callers untouched: they branch to a
    terminal label immediately after the helper, so r4 going from "struct
    ptr" to "fd" is harmless. T5 / T9 callers chain multiple emits and
    rely on r4 = struct ptr throughout (used as `r4 + 8 = conn` arg to
    every PLT_reg_notievent_*_rsp). Those callers MUST pass fd_reg=6 to
    avoid clobbering the struct ptr; r6 is otherwise unused in T5 / T9
    bodies and is callee-saved.

    `scratch_sp_offset` is a 1-byte stack region the byte_value is
    written to first (so we can pass &sp[off] as the write source). Uses
    strb (1 byte) not str (4 bytes) so the scratch location can safely
    overlap state bytes we no longer need without clobbering adjacent
    state we still need. `fail_label` is the branch target if open()
    fails; the rest of the block falls through after close().
    """
...
def _emit_t9(a: Asm) -> None:
    ...
    # Subscription gate (§6.7.1 strict): emit only if sub_battery armed.
    # Cleared after emit; CT re-registers for next CHANGED.
    ...
    # Subscription gate (§6.7.1 strict): emit CHANGED only if T8 INTERIM has
    # armed sub_papp (state[15] = 1). Cleared after emit; CT re-registers.
    ...
    # AVRCP §6.7.1 strict: clear sub_pos (state[13]) after CHANGED. CT must
    # re-RegisterNotification(0x05) to receive the next emit. CT-side cadence
    # of re-registers (≈1 Hz on Bolt-on-Pixel) effectively sets the wire-side
    # POS_CHANGED rate. r4 holds struct ptr — use fd_reg=6.
