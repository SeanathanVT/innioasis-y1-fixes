...
| **`patch_y1_apk.py`** | `com.innioasis.y1*.apk` | Smali patches A/B/C for Artist→Album navigation, and **Patch E** (iter22d, refined iter25) splitting `PlayControllerReceiver`'s short-press `KEY_PLAY → playOrPause()` branch into three discrete arms per AVRCP 1.4 §11.1.2: `KEY_PLAY` (85, `KEYCODE_MEDIA_PLAY_PAUSE`) → `playOrPause()` (toggle, legacy MediaButton); `KEYCODE_MEDIA_PLAY` (126, from PASSTHROUGH 0x44 via `KEY_PLAYCD`) → `play(false)` (discrete PLAY); `KEYCODE_MEDIA_PAUSE` (127, defensively wired) → `pause(0x12, true)` (discrete PAUSE). Closes the iter22d→iter23 Bolt-EV "discrete PLAY when already playing = no-op" symptom — toggle inverted Bolt's intent, discrete `play()` honors §11.1.2's "transition to PLAYING from any state". Uses androguard + apktool. *(Patch D / iter21 — the music-app FF/RW hold-loop cap — was reverted in iter24: U1 in `patch_libextavrcp_jni.py` fixes the AVRCP-side root cause at the kernel input layer, and iter21's 5-second cap unhelpfully bounded local hardware-button hold-FF too, breaking long scrubs through audiobooks/DJ mixes.)* |
