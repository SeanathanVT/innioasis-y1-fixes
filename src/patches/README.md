...
| **`patch_libextavrcp_jni_minimal.py`** | `libextavrcp_jni.so` | Currently no-op (J1 rolled back 2026-05-05 — routed to wrong dispatch). Trampoline patch under design: code-cave that calls `btmtk_avrcp_send_get_capabilities_rsp` via PLT directly. Mutually exclusive with `patch_libextavrcp_jni.py`. Wired by `apply.bash --avrcp-min`. |
