...
| **`patch_y1_apk.py`** | `com.innioasis.y1*.apk` | Smali patches A/B/C for Artist→Album navigation, and **Patch E** (iter22d, refined iter25, +STOP iter27) splitting `PlayControllerReceiver`'s short-press `KEY_PLAY → playOrPause()` branch into four discrete arms per AVRCP 1.3 §4.6.1 + AV/C Panel Subunit Spec [ref 2] + ICS Table 8 (op_id status for Cat 1 TGs, `docs/spec/AVRCP.ICS.p17.pdf` §1.5): `KEY_PLAY` (85, `KEYCODE_MEDIA_PLAY_PAUSE`) → `playOrPause()` (toggle, legacy MediaButton); `KEYCODE_MEDIA_PLAY` (126, from PASSTHROUGH 0x44 via `KEY_PLAYCD`) → `play(false)` (discrete PLAY, ICS Table 8 item 19 mandatory); `KEYCODE_MEDIA_PAUSE` (127, from PASSTHROUGH 0x46 via `KEY_PAUSECD`) → `pause(0x12, true)` (discrete PAUSE, item 21 optional); **`KEYCODE_MEDIA_STOP`** (86, from PASSTHROUGH 0x45 via `KEY_STOPCD`) → **`stop()V`** (discrete STOP, **item 20 mandatory — closes a verified ICS compliance gap, iter27**). Closes the iter22d→iter23 Bolt-EV "discrete PLAY when already playing = no-op" symptom — toggle inverted Bolt's intent, discrete `play()` honors AV/C Panel Subunit Spec op_id 0x44's "transition to PLAYING from any state" (concrete frame in AVRCP 1.3 §19.3 Appendix D). Uses androguard + apktool. *(Patch D / iter21 — the music-app FF/RW hold-loop cap — was reverted in iter24: U1 in `patch_libextavrcp_jni.py` fixes the AVRCP-side root cause at the kernel input layer, and iter21's 5-second cap unhelpfully bounded local hardware-button hold-FF too, breaking long scrubs through audiobooks/DJ mixes.)* |
