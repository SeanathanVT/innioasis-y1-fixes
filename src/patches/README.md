...
| **`patch_libextavrcp_jni_minimal.py`** | `libextavrcp_jni.so` | J1 (single byte at file 0x6526: `cmp.w lr, #8` → `cmp.w lr, #9`) — routes size-9 msg-519 frames (produced by P1 for VENDOR_DEPENDENT) into the JNI's existing BT-SIG VENDOR path → reaches a `JNIEnv->CallVoidMethodV` invocation into Java's `*Ind` callback at file 0x65a4 instead of the "unknow indication" default-reject. Mutually exclusive with `patch_libextavrcp_jni.py`. Wired by `apply.bash --avrcp-min`. |
