...
| **`patch_libextavrcp_jni_minimal.py`** | `libextavrcp_jni.so` | R1 (redirect `bne.n 0x65bc` → `bl.w 0x7308`), T1 (GetCapabilities trampoline at 0x7308), T2 (RegisterNotification(TRACK_CHANGED) trampoline at 0x72d0), T4 stub (8 bytes at 0xac54 — restores r0 + falls through to "unknow indication" so unhandled PDUs emit msg=520 NOT_IMPLEMENTED), and **LOAD #1 program-header extension** (FileSiz/MemSiz 0xac54 → 0xac5c, reusing the page-alignment zero padding between LOAD #1 and LOAD #2 to host T4). T1 hardware-verified iter5; T2 hardware-verified iter6. Pairs with P1 in `patch_mtkbt_minimal.py`. Mutually exclusive with `patch_libextavrcp_jni.py`. Wired by `apply.bash --avrcp-min`. |
