...
| **`patch_y1_apk.py`** | `com.innioasis.y1*.apk` | Smali patches A/B/C for Artist→Album navigation, and **Patch E** (iter22d) expanding `PlayControllerReceiver`'s `KEY_PLAY → playOrPause()` branch to also accept `KEYCODE_MEDIA_PLAY` (126) and `KEYCODE_MEDIA_PAUSE` (127), covering CTs that issue discrete PASSTHROUGH 0x44 PLAY (per AVRCP 1.4 §11.1.2) which `libextavrcp_jni` maps to `KEYCODE_MEDIA_PLAY` rather than the `KEYCODE_MEDIA_PLAY_PAUSE` toggle. Uses androguard + apktool. *(Patch D / iter21 — the music-app FF/RW hold-loop cap — was reverted in iter24: U1 in `patch_libextavrcp_jni.py` fixes the AVRCP-side root cause at the kernel input layer, and iter21's 5-second cap unhelpfully bounded local hardware-button hold-FF too, breaking long scrubs through audiobooks/DJ mixes.)* |
