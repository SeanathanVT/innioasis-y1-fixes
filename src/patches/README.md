...
| **`patch_libextavrcp_jni_minimal.py`** | `libextavrcp_jni.so` | R1 (redirect `bne.n 0x65bc` → `bl.w 0x7308`), T1 (GetCapabilities response trampoline at 0x7308 — overwrites `testparmnum`), T2 (RegisterNotification(TRACK_CHANGED) trampoline at 0x72d0 — overwrites `classInitNative`), T4 (GetElementAttributes 3-attribute response trampoline at vaddr 0xac54 — uses LOAD #1 page-padding region), and **LOAD #1 program-header extension** (FileSiz/MemSiz 0xac54 → 0xace8 to map T4 as R+E). T1 hardware-verified iter5, T2 iter6, T4 stub iter9, T4 Title-only iter11 ("Y1 Test" displayed on Sonos), T4 multi-attribute iter12 (3:1 frames, only first picked up), T4 single-frame iter13 (correct multi-attr semantics) pending. See [`ARCHITECTURE.md`](../../docs/ARCHITECTURE.md). Pairs with P1 in `patch_mtkbt_minimal.py`. Mutually exclusive with `patch_libextavrcp_jni.py`. Wired by `apply.bash --avrcp-min`. |
