...
| **`patch_libextavrcp_jni_minimal.py`** | `libextavrcp_jni.so` | R1 (redirect `bne.n 0x65bc` at file 0x6538 → `bl.w 0x7308`), T1 (40-byte GetCapabilities trampoline at 0x7308 — overwrites unused `testparmnum`, calls `…send_get_capabilities_rsp` via PLT 0x35dc), T2 (48-byte block at 0x72d0 — classInitNative stub + RegisterNotification(TRACK_CHANGED) trampoline calling `…send_reg_notievent_track_changed_rsp` via PLT 0x3384). T1 hardware-verified iter5 (Sonos progressed past GetCapabilities). Pairs with P1 in `patch_mtkbt_minimal.py`. Mutually exclusive with `patch_libextavrcp_jni.py`. Wired by `apply.bash --avrcp-min`. |
