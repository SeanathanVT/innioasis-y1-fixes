...
|---|---|---|
| **`patch_mtkbt.py`** | `mtkbt` — SDP shape (V1/V2/V3/V4/V7/V8/S1), activeVersion override (V6), force-PASSTHROUGH-emit op_code dispatch (P1), best-effort AVDTP sig 0x0c → sig 0x02 alias (V5), M1 (widens the RegNotif INTERIM/CHANGED dispatcher cmp from 1 to 0x0F so wire ctype matches the JNI's reasonCode), M2 + M3 (Path A: bypass outbound-frame builder's chip-readiness list-check + chip-busy flag set for fragmented multi-frame responses like `msg=540` GetElementAttributes), M4 (Path B: same list-check bypass on the structurally-identical twin builder for short single-PDU responses like `msg=544` RegNotif INTERIM/CHANGED), and M5 (Path B: re-encode the short-frame builder's wire-byte-0 source so the AVCTP header matches Path A's working pattern, fixing subscription-class CT acceptance of RegNotif INTERIM responses). | `--avrcp` |
