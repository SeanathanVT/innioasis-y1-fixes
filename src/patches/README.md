...
| **`patch_y1_apk.py`** | `com.innioasis.y1*.apk` | Smali patches A/B/C for Artist→Album navigation, plus **Patch D** (iter21) bounding the FF/RW hold-loop in `PlayerService$startFastForward$1`/`$startRewind$1` to **50 iterations × 100 ms ≈ 5 s** and clearing `fastForwardLock` on cap (also flips `fastForwardLock` `private`→`public` so the lambda can `iput-boolean`). Defensive recovery for dropped PASSTHROUGH-release frames under AVCTP saturation — see iter19c/iter20b TV captures. Uses androguard + apktool. |
