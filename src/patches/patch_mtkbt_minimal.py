...
goal is to make Sonos (and other AVRCP 1.3+ controllers) engage with the
record and start sending VENDOR_DEPENDENT commands; the larger
patch_mtkbt.py set tries to advertise AVRCP 1.4 with `0x0033` features but
ships a record that's structurally incomplete (no ServiceName, no Provider,
no secondary CT record), and Sonos refuses to engage with it.

This minimal set targets the empirically-working Pixel-1.3 SDP shape (per
Trace #11 reference data + the 2026-05-05 sdptool A/B against Pixel 4) plus
the one structural attribute that even Pixel-1.3 has and Y1 lacks at every
patch level: a 0x0100 ServiceName.
...
Status: research probe. Whether Sonos engages a record with this shape is
the question this patcher exists to answer. If yes, move to the inbound-
COMMAND response side (which mtkbt's compiled-1.0 dispatcher does NOT
handle — see INVESTIGATION.md "Conclusion (2026-05-04)" for the trampoline-
into-userspace plan). If no, the SDP gate is something else (likely the
absence of the secondary 0x110e+0x110f record).

Usage:
    python3 patch_mtkbt_minimal.py mtkbt
    python3 patch_mtkbt_minimal.py mtkbt --output /tmp/mtkbt.patched
    python3 patch_mtkbt_minimal.py mtkbt --verify-only
"""
...
def main():
    ...
    if not args.skip_md5 and input_md5 != STOCK_MD5:
        ...
        print("       This patcher targets stock mtkbt only — it is not compatible")
        print("       with the output of patch_mtkbt.py (the larger --avrcp set).")
