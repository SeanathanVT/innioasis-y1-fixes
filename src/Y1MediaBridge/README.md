...
Ship alongside the patched binaries (current `--avrcp` stack — advertises **AVRCP 1.3** over AVCTP 1.2; `mtkbt`'s shipped command-handling layer is internally 1.0 and the trampoline chain in `libextavrcp_jni.so` synthesises the 1.3 responses on top):
