...
**AVRCP metadata over BT is working under `--avrcp`** (as of 2026-05-05, iter13 hardware-verified): Title + Artist + Album simultaneously displayed on Sonos's Now Playing screen — first time AVRCP 1.3 metadata has been delivered to a peer device from this firmware. `--avrcp` advertises AVRCP **1.3** over AVCTP **1.2** in the SDP record (V1/V2 patches in `patch_mtkbt.py`); 1.3 is the highest version where the SDP advertisement, AVCTP, and our trampoline coverage all line up — `mtkbt`'s shipped command-handling layer is internally AVRCP 1.0 and rejects anything past `PASSTHROUGH` natively, so the trampoline chain in `libextavrcp_jni.so` synthesises the 1.3 responses directly. iter14b wires the strings to real metadata via Y1MediaBridge (file-based handoff to a native trampoline in libextavrcp_jni.so). iter15 added state-tracked TRACK_CHANGED CHANGED notifications but deadlocked Sonos by returning a real track_id in INTERIM. iter16 pinned the wire-level track_id in INTERIM/CHANGED to the AVRCP spec's `0xFF×8` "no stable identity" sentinel — protocol exchange fully working, but Sonos's idle GetElementAttributes poll rate is too slow for shuffle. iter17a adds a third trampoline (T5) reached via the patched `notificationTrackChangedNative` JNI method so Y1MediaBridge's track-change broadcast emits CHANGED on the AVRCP wire within milliseconds, independent of Sonos's polling cadence. iter17a hardware test (2026-05-06) showed the proactive layer working but Sonos rendering metadata field-by-field with visible flicker — diagnosed from logcat msg-id ratios as a regression of the iter12 multi-attribute bug iter13 had originally fixed (T4 was emitting 3 separate msg=540 frames instead of one packed frame). iter17b (current, awaiting hardware verification) restores iter13's `arg2=index, arg3=total` calling convention to `btmtk_avrcp_send_get_element_attributes_rsp`, packing Title + Artist + Album into one frame.
...
- [docs/AVRCP13-COMPLIANCE-PLAN.md](docs/AVRCP13-COMPLIANCE-PLAN.md) — staged plan from the current Sonos-correct subset to full AVRCP 1.3 spec compliance (Phases A–E, ~11–17 days end-to-end)
