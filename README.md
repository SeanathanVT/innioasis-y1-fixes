...
- **2026-05-03** – Import the [Y1MediaBridge](https://github.com/SeanathanVT/Y1MediaBridge) Android service app into this repo as `src/Y1MediaBridge/` via `git subtree add --prefix=src/Y1MediaBridge /path/to/Y1MediaBridge HEAD`. The merge commit grafts Y1MediaBridge's full commit history into this repo's log under that prefix; `git log src/Y1MediaBridge/` shows the imported history. Y1MediaBridge.apk is still treated as an externally-built artifact by the bash (must be staged into `--artifacts-dir` for `--avrcp`); the in-tree source enables building it locally rather than pulling a pre-built APK from elsewhere. Build via `cd src/Y1MediaBridge && ./gradlew assembleDebug` (or `assembleRelease`); output APK lands under `src/Y1MediaBridge/app/build/outputs/apk/`. The standalone `/work/Y1MediaBridge/` repo at its previous location can keep getting pushes; future pulls into the monorepo would use `git subtree pull --prefix=src/Y1MediaBridge ...`. The monorepo and the standalone repo are currently equivalent at Y1MediaBridge HEAD `b2773e15`.
