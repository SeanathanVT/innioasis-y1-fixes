...
- **`patch_mtkbt.py`**
  ...
  - **Thirteen patches applied:**
    ...
    - **E5** `0x309ed`: `0xd1` → `0xe0` — `BNE 0x30aca` → `B 0x30aca` (unconditional) in op_code=4 (GetCapabilities) dispatcher at `0x3096C`. Forces all inbound GetCapabilities dispatch through the AVRCP 1.3/1.4 init path (`0x02fd34` → 5-slot init + AVAILABLE_PLAYERS) regardless of mtkbt's internal `[conn+0x149]` version classification. Encoding miracle: T1 `bne #+218` (`6d d1`) and T2 narrow `b.n #+218` (`6d e0`) share the same numeric offset, so this is a single-byte patch.
    - **E7a** `0x033dec`: `0x90` → `0x94` — `movs r0, #0x90` → `movs r0, #0x94` at the no-SDP-CT-record fallback site. Changes the default AVRCP version classification stored in `[conn+0x5d9]` from `0x90` (1.0) to `0x94` (1.4) when the remote doesn't advertise an AVRCP CT (UUID 0x110e) record — which is the common case for car infotainment systems.
    - **E7b** `0x034100`: `0x90` → `0x94` — same patch at the second identical fallback site (different op_code branch).
  ...
  - **E5 note:** After E3/E4 made the SDP advertisement textbook-1.4, `cardinality` was *still* 0 and the JNI logs showed *no* inbound AVRCP `Recv AVRCP indication` msg_ids beyond connect/disconnect/activate. The dispatcher at `0x3096C` (the op_code=4 GetCapabilities path) reads `[conn+0x149] & 0x7f`, branches to the 1.3/1.4 init path (`0x02fd34`) only when the value is *not* `0x10` (1.0). For our car, mtkbt was apparently classifying the connection as 1.0 — likely because the car's CT-side SDP record (UUID 0x110e) does not advertise 1.4 — and the 1.0 path skips the 5-slot init and AVAILABLE_PLAYERS notification that primes the controller for 1.4. E5 forces *all* op_code=4 dispatches through the 1.3/1.4 init path, removing mtkbt's reliance on the remote's CT version classification. NOT to be confused with the eliminated **E2** patch at the same instruction (`bne → nop`), which went the wrong direction (made everything fall through to 1.0).
  - **E7 note:** E5 alone wasn't sufficient because the dispatcher at `0x3096C` is only entered when an inbound AVRCP command is forwarded from mtkbt to the JNI dispatch socket — and for "AVRCP 1.0" classified connections, mtkbt processes commands entirely through an internal 1.0 handler that never forwards to the JNI. The version classification has to be flipped *before* command dispatch. Pattern-search of all immediate writes to `[conn+0x5d9]` (the source field copied to `[conn+0x149]`) revealed exactly two `movs r0, #0x90` writes, both gated on `cbnz [r4+0x5dc]` — the "no SDP CT record found" fallback. Cars typically don't advertise 0x110e, so this fallback fires and the connection classifies as 1.0. E7a/E7b flip both immediates from `0x90` to `0x94`, making the fallback decode to 0x14 (1.4) instead of 0x10 (1.0). Other immediate writes to `+0x5d9` (`0xa0`/`0xc0`/`0xd0` etc.) fire on specific peer-state branches, not the no-SDP fallback, and are deliberately left alone.
  - Stock MD5: `3af1d4ad8f955038186696950430ffda` — Output MD5: `ff50024bc851395408353ba52d140790`
...
- **`patch_libextavrcp_jni.py`**
  - Patches `libextavrcp_jni.so` to force `g_tg_feature=14` (AVRCP 1.4) and `sdpfeature=0x23`; raises the GetCapabilities event count cap in `getCapabilitiesRspNative` from 13 to 14 so the car CT sees the full AVRCP 1.4 event list and sends `REGISTER_NOTIFICATION`
  - 4 ARM Thumb2 instruction overwrites: 2 in `BluetoothAvrcpService_activateConfig_3req` at 0x375c (hardcode tg_feature/sdpfeature, bypassing bitmask logic), 2 in `getCapabilitiesRspNative` (`FUN_005de8`) at 0x5e56/0x5e5c (raise GetCapabilities event count cap from 13 to 14 — the cardinality fix; the actual CONNECT_CNF handler is at 0x62EA and does not gate on tg_feature)
