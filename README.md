...
**AVRCP metadata over BT is working under `--avrcp-min`** (as of 2026-05-05, iter13 hardware-verified): Title + Artist + Album simultaneously displayed on Sonos's Now Playing screen — first time AVRCP 1.3 metadata has been delivered to a peer device from this firmware. (`--avrcp-min` advertises AVRCP **1.3** over AVCTP **1.2** in the SDP record — the V1/V2 minimal patches in `patch_mtkbt_minimal.py`. The legacy `--avrcp` flag tried to advertise 1.4 with `0x0033` features but `mtkbt`'s shipped command-handling layer is internally 1.0 and rejected anything past `PASSTHROUGH`; 1.3 is the highest version where the SDP advertisement, AVCTP, and our trampoline coverage all line up.) iter14b wires the strings to real metadata via Y1MediaBridge (file-based handoff to a native trampoline in libextavrcp_jni.so). iter15 added state-tracked TRACK_CHANGED CHANGED notifications but deadlocked Sonos by returning a real track_id in INTERIM (Sonos then waits for CHANGED before re-polling, and our trampolines are reactive — chicken-and-egg). iter16 pinned the wire-level track_id in INTERIM/CHANGED to the AVRCP spec's `0xFF×8` "no stable identity" sentinel — protocol exchange fully working, but Sonos's idle GetElementAttributes poll rate is too slow for shuffle (display freezes a couple audioIds behind reality). iter17a adds a third trampoline (T5) reached via the patched `notificationTrackChangedNative` JNI method so Y1MediaBridge's track-change broadcast emits CHANGED on the AVRCP wire within milliseconds, independent of Sonos's polling cadence. iter17a hardware test (2026-05-06) showed the proactive layer working but Sonos rendering metadata field-by-field with visible flicker — diagnosed from logcat msg-id ratios as a regression of the iter12 multi-attribute bug iter13 had originally fixed (T4 was emitting 3 separate msg=540 frames instead of one packed frame). iter17b (current, awaiting hardware verification) restores iter13's `arg2=index, arg3=total` calling convention to `btmtk_avrcp_send_get_element_attributes_rsp`, packing Title + Artist + Album into one frame.
