...
- **AVRCP 1.3 metadata + control over Bluetooth** — peer Bluetooth Controller (car head unit, TV, smart speaker) sees Title / Artist / Album / Genre / TrackNumber / TotalNumberOfTracks / PlayingTime, live play status with millisecond-precision position, play-state and track-change notifications, battery status, and Repeat / Shuffle (bidirectional). Behind `--avrcp` (left as opt-in because it ships a much larger set of byte-level patches than the other flags). Architecture: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). Compliance scorecard: [`docs/BT-COMPLIANCE.md`](docs/BT-COMPLIANCE.md).
...
`--all` = `--adb --bluetooth --music-apk --remove-apps --root`. `--avrcp` is left as opt-in (see [Status](#status)).
...
`--avrcp` ships a much larger set of byte-level patches than the other flags and modifies the music APK's manifest (adding the `com.android.music.MediaPlaybackService` intent-filter binder). Opt in explicitly:
...
| `--avrcp` | AVRCP 1.3 metadata pipeline: patches `mtkbt`, `libextavrcp_jni.so`, `MtkBt.odex`, and the music app (smali + manifest). Left as opt-in (ships a much larger set of byte-level patches than the other flags). Patch ID legend in [`docs/PATCHES.md`](docs/PATCHES.md); architecture in [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). |
...
- `--avrcp` shares the same Python + Java toolchain as `--music-apk`; no additional dependencies.
