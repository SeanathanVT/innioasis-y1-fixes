...
**AVRCP metadata over BT is working under `--avrcp-min`** (as of 2026-05-05, iter13 hardware-verified): Title + Artist + Album simultaneously displayed on Sonos's Now Playing screen — first time AVRCP 1.4 metadata has been delivered to a peer device from this firmware. iter14b wires the strings to real metadata via Y1MediaBridge (file-based handoff to a native trampoline in libextavrcp_jni.so). iter15 added state-tracked TRACK_CHANGED CHANGED notifications but deadlocked Sonos by returning a real track_id in INTERIM (Sonos then waits for CHANGED before re-polling, and our trampolines are reactive — chicken-and-egg). iter16 (current, awaiting hardware verification) keeps the same architecture but pins the wire-level track_id in INTERIM/CHANGED to AVRCP 1.4 §6.7.2's `0xFF×8` "no stable identity" sentinel — Sonos stays in poll-on-each-event mode while CHANGED edges still invalidate the cache when the track actually moves.
