...
| **H1, H2, H3** | `/sbin/adbd` (in `boot.img` ramdisk) | Change the *argument values* of the three calls in adbd's drop_privileges block at vaddr `0x94b8` from `2000` (AID_SHELL) / `11` (gid count) to `0`. The syscalls still execute (so all bionic bookkeeping — capability bounding-set, thread-credential sync — runs normally), but the process ends up at uid=0/gid=0 with no supplementary groups. After flashing, `adb shell` returns uid 0 directly. Required because this OEM adbd ignores `ro.secure`/`service.adb.root`/`ro.debuggable` for the privilege-drop decision. An earlier NOP-the-blx revision (NOPed each 4-byte BLX with `movs r0,#0; nop`) caused "device offline" — the bionic setuid wrapper at `0x19418` does work that downstream adbd code depends on. |
