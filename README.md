...
- **`innioasis-y1-fixes.bash`** (v1.8.0)
  ...
  - **`--root` (re-added v1.8.0)** installs the prebuilt `su/build/su` setuid-root binary at `/system/xbin/su` (mode 06755, root:root). `/sbin/adbd` is untouched — root is obtained post-flash by `adb shell /system/xbin/su`. Requires `cd su && make` once before first use; the bash exits with a clear error pointing at `make` if the prebuilt is missing. Replaces the v1.3.0–v1.6.0 approach of byte-patching `/sbin/adbd` in the boot.img ramdisk, which caused "device offline" on hardware. Historical scripts `patch_adbd.py` and `patch_bootimg.py` remain in the tree as documentation but are not invoked.
...
- **`su/`** (root, v1.8.0)
  ...
  - **`su/su.c`** — direct ARM-EABI syscall implementation, no libc dependency. `setgid(0)` → `setuid(0)` → `execve("/system/bin/sh", ...)`. Supports three invocation forms: bare `su` (interactive root shell), `su -c "<cmd>"` (one-off), and `su <prog> [args...]` (exec-passthrough).
  - **`su/start.S`** — ~10-line ARM Thumb-2 entry stub; extracts argc/argv/envp from the ELF process-start stack layout, calls `main`, exits with main's return value via `__NR_exit`.
  - **`su/Makefile`** — cross-compile via `arm-linux-gnu-gcc` (EPEL package `gcc-arm-linux-gnu`). `-nostdlib -ffreestanding -static -Os -mthumb -mfloat-abi=soft`; output ~900 bytes, statically linked, no `NEEDED` entries. Install toolchain on Rocky/Alma/RHEL/Fedora: `sudo dnf install -y epel-release && sudo dnf install -y gcc-arm-linux-gnu binutils-arm-linux-gnu make`.
  ...
  - **Build:** `cd su && make` produces `su/build/su`. The bash references this prebuilt path; if missing, `--root` exits with a clear error pointing at `make`. Idempotent (re-running `make` is a no-op if sources are unchanged).
  - **Deploy:** the bash's `--root` flag does `install -m 06755 -o root -g root su/build/su /system/xbin/su` against the mounted system.img. Post-flash: `adb shell /system/xbin/su -c "id"` → `uid=0(root)`.
...
- **`patch_bootimg.py`** *(unwired since v1.7.0; kept as historical record)*
  ...
  - **Status:** unwired since v1.7.0 because the H1/H2/H3 adbd byte patches caused "device offline" on hardware. Superseded in v1.8.0 by the `/system/xbin/su` install approach (see `su/` above), which leaves `/sbin/adbd` untouched.
...
- `/system/xbin/su` – Setuid-root escalator (mode 06755, root:root) installed by `--root`. Built from `su/su.c` + `su/start.S` via `arm-linux-gnu-gcc`; ~900-byte direct-syscall ARM-EABI ELF, no libc, no manager APK
...
- For `--root` only: prebuilt `su/build/su` (run `cd su && make` once). The Makefile uses `arm-linux-gnu-gcc` from EPEL — install via `sudo dnf install -y epel-release && sudo dnf install -y gcc-arm-linux-gnu binutils-arm-linux-gnu make` on Rocky/Alma/RHEL/Fedora, or the equivalent `gcc-arm-linux-gnueabi` package on Debian/Ubuntu.
...
- `su/build/su` *is* consumed by the bash's `--root` flag (v1.8.0+); the binary must exist at that path before `--root` runs.
...
- `--root` – Install the prebuilt `su/build/su` setuid-root binary at `/system/xbin/su` (mode 06755, root:root). Run `cd su && make` once before first use. Post-flash, root is obtained via `adb shell /system/xbin/su` (or `/system/xbin/su -c "<cmd>"`).
