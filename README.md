...
- **`patch_mtkbt.py`**
  ...
  - **Twelve patches applied:**
    ...
    - **G1** `0x675c0`: 20 bytes (Thumb mode) — replaces the `[AVRCP]/[AVCTP]` xlog wrapper prologue with a NULL-guarded thunk that tail-calls `__android_log_print(LOG_INFO, fmt, fmt, varargs...)` to redirect daemon-side log output to logcat. **Diagnostic-only.** See G1 note below.
...
- **2026-05-02** – Re-add **G1 with NULL guard** to `patch_mtkbt.py` (no G2). 20-byte Thumb thunk at `0x675c0`: `cbz r2, .L_null; movs r0,#4; mov r1,r2; ldr.w pc,[pc,#4]; nop; .word 0xaef8; .L_null: movs r0,#0; bx lr`. The CBZ guards against NULL fmt pointers from xlog callsites that crashed the previous attempt. G2 (PLT redirect) deliberately omitted — G1 alone catches the 2988 wrapper callsites which is where the AVRCP/AVCTP diagnostic surface lives; the 1091 direct PLT callers are kernel-side BT stack code we don't need to see (and likely contains the population of NULL-passing offenders). New patched MD5: `e2f9033eb50f10d2fc274726edb3ca75`.
